...
|Capability|Description|Values|
|----------|-----------|------|
| `deviceType` |  Specify the target test device. This cap must be `tv` to run tests on Apple TV. Defaults `undefined`. The behaviour of `undefined`, `iphone` and `ipad` is the same. | `iphone`, `ipad`, `tv` |
