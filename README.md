...
|`appium:forceTurnOnSoftwareKeyboard`|Set this option to `true` in order to turn software keyboard on since Appium 1.22.0. This option helps to avoid `Keyboard is not present` error. It is set to `true` by default. Appium respect pre-set simulator keyboard preference when this value is `false`. |`true` or `false`|
...
forceTurnOnSoftwareKeyboard | boolean | See the description of the corresponding capability.
