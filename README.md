...
|`simulatorPasteboardAutomaticSync`| Handle the `-PasteboardAutomaticSync` flag when simulator process launches. It could improve launching simulator performance not to sync pasteboard with the system when this value is `off`. `On` forces the flag enabled. `System` does not provide the flag to the launching command. `on`, `off`, or `system` is available. Defaults to `off` | e.g. `system` |
