...
class WebDriverAgent {
  ...
  async waitForStart (startTime) {
    ...
    let startDetector = (stdout) => {
      ...
      lineCount++;
      let threshold = this.realDevice ? 1000 : 200;
  ...
  async killHangingProcesses () {
    ...
    if (this.realDevice) {
      await killAppUsingAppName(this.device.udid, this.realDeviceLogger);
      await killAppUsingAppName(this.device.udid, `iproxy`);
    } else {
      await killAppUsingAppName(this.device.udid, `tail`);
      await killAppUsingAppName(this.device.udid, `XCTRunner`); // Eventhough xcodebuild is killed. XCTRunner still keeps hanging.
    }
  }
