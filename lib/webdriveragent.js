...
const IDEVICESYSLOG_PATH = 'idevicesyslog';
...
class WebDriverAgent {
  ...
  constructor (args = {}) {
    ...
    this.realDeviceLogger = args.realDeviceLogger || IDEVICESYSLOG_PATH;
  ...
  async launch (sessionId) {
    ...
    await this.destroyXcodeProcesses();
  ...
  async destroyXcodeProcesses () {
    try {
      let {stdout} = await exec('ps', ['-ax']);
      let match = /(\d+)\s.+?(?=xcodebuild)/.exec(stdout);
      if (match) {
        log.info(`killing xcodebuild process: ${match[1]}`);
        process.kill(match[1]);
      }
    } catch (err) {
      log.error(`Could not get xcodebuild processes: ${err}`);
    }
  }
