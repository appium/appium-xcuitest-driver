...
const AGENT_STARTED_REGEX = /WebDriverAgent started on port(.*)/;
...
class WebDriverAgent {
  ...
  async launch (sessionId) {
    return new B(async (resolve, reject) => {
      ...
      log.info(`WebDriverAgent started on port ${port}`);

      this.port = port;

      this.jwproxy = new JWProxy({host: 'localhost', port: this.port, base: ''});
