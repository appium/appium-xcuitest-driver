...
class WebDriverAgent {
  ...
  async startXcodebuild () {
    ...
    return await new B(async (resolve, reject) => {
      ...
      try {
        let agentUrl;
        let xcodeBuildFinishedTimestamp = null;
        let matchesCountBeforeBuildFinished = 0;
        let lineCount = 0;
        let showWaitingMessage = true; // turn off logging once we have hit the end

        let startDetector = (stdout) => {
          const matches = stdout.match(AGENT_STARTED_REGEX);
          if (xcodeBuildFinishedTimestamp === null) {
            matchesCountBeforeBuildFinished = matches ? matches.length : 0;
          } else {
            const matchesCountAfterBuildFinished = matches ? matches.length : 0;
            if (matchesCountAfterBuildFinished > matchesCountBeforeBuildFinished) {
              const agentUrlMatch = AGENT_STARTED_REGEX.exec(matches[matchesCountAfterBuildFinished - 1]);
              if (!agentUrlMatch || agentUrlMatch.length < 2) {
                log.errorAndThrow(new Error('No url detected from WebDriverAgent'));
              }
              agentUrl = agentUrlMatch[1];
          ...
          if (xcodeBuildFinishedTimestamp !== null) {
            const secondsElapsed = process.hrtime(xcodeBuildFinishedTimestamp)[0];
            if (secondsElapsed > WDA_STARTUP_TIMEOUT_SEC) {
              log.errorAndThrow(new Error(`WebDriverAgent has not been detected as running after ${secondsElapsed} seconds`));
            }
          }
        ...
        const wdaStartupDetector = this.deviceLogs.start(startDetector);
        await this.xcodebuild.start();
        xcodeBuildFinishedTimestamp = process.hrtime();
        log.info(`Waiting maximum ${WDA_STARTUP_TIMEOUT_SEC} seconds for WebDriverAgent to start on the device...`);
        await wdaStartupDetector;
        log.info(`WebDriverAgent has been successfully started at url '${agentUrl}'`);
        resolve(agentUrl);
      } catch (err) {
