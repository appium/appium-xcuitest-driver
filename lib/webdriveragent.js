  ...
  async startXcodebuild () {
    ...
    return await new B(async (resolve, reject) => {
      ...
      try {
        ...
        let isXcodeBuildStarted = false;
        let matchesCountBeforeBuildStarted = 0;
        let lineCount = 0;
        let showWaitingMessage = true; // turn off logging once we have hit the end

        let startDetector = (stdout) => {
          let matchesCountAfterBuildStarted = 0;
          if (isXcodeBuildStarted) {
            if (null === timestampStarted) {
              timestampStarted = process.hrtime();
            }
            let match = null;
            let recentMatch = null;
            do {
                match = AGENT_STARTED_REGEX.exec(stdout);
                if (match !== null) {
                  recentMatch = match;
                }
                ++matchesCountAfterBuildStarted;
            } while (match !== null);
            if (recentMatch !== null && matchesCountAfterBuildStarted > matchesCountBeforeBuildStarted) {
              agentUrl = recentMatch[1];
              if (!agentUrl) {
            ...
            matchesCountBeforeBuildStarted = 0;
            while (AGENT_STARTED_REGEX.exec(stdout) !== null) {
              ++matchesCountBeforeBuildStarted;
            }
          }

          if (timestampStarted !== null) {
            const secondsElapsed = process.hrtime(timestampStarted)[0];
        ...
        // give logger process some time to wind up
        B.delay(1000);
        isXcodeBuildStarted = true;
