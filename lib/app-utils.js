 ...
 * @returns {Any | undefined} Either the extracted value or undefined if no such key has been found in the plist.
...
async function extractPlistEntry (app, entryName) {
  ...
  let plist;
  if (_.isNil(this)) {
    plist = await parseFile();
  } else {
    plist = PLIST_CACHE.get(this)?.[app];
    if (!plist) {
      plist = await parseFile();
