...
export async function verifyApplicationPlatform(app, expectedPlatform) {
  ...
  if (isSimulator && os.arch() !== 'arm64') {
    ...
    if (_.includes(stdout, 'arm64')) {
      ...
      throw new Error(`The '${appFileName}' (${bundleId}) application is built for arm64 Simulator ` +
        `architecture (${_.trim(stdout)}}) and is not supported on the current ${os.arch()} platform. ` +
        `Please rebuild your application to support the ${os.arch()} platform.`
      );
    }
  }
}
