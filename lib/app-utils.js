...
export async function verifyApplicationPlatform() {
  ...
  const appFileName = path.basename(this.opts.app);
  ...
  if (
    ...
    const bundleId = await this.appInfosCache.extractBundleId(this.opts.app);
    throw new Error(
...
export async function parseLocalizableStrings(opts) {
  ...
  try {
    if (isArchive) {
      ...
      bundleRoot = _.first(await findApps(tmpRoot, [APP_EXT]));
    }

    let lprojRoot;
    for (const subfolder of [`${language}.lproj`, localizableStringsDir, '']) {
      lprojRoot = path.resolve(bundleRoot, subfolder);
    ...
    this.log.info(`Got ${resourcePaths.length} resource file(s) in '${lprojRoot}'`);
    ...
    for (const resourcePath of resourcePaths) {
      ...
      try {
        ...
        this.log.debug(`Parsed ${_.keys(data).length} string(s) from '${resourcePath}'`);
    ...
    this.log.info(`Got ${_.keys(resultStrings).length} string(s) from '${lprojRoot}'`);
 ...
 * @returns {Promise<IpaInfo>}
 */
export async function downloadIpa(stream, headers) {
  let fileName = `appium-app-${new Date().getTime()}${IPA_EXT}`;
  if (_.isString(headers['content-disposition']) && /^attachment/i.test(headers['content-disposition'])) {
  ...
  if (fileName.toLowerCase().endsWith(ZIP_EXT)) {
    ...
    try {
      ...
      for (const matchedPath of matchedPaths) {
        ...
        const {size} = await fs.stat(isolatedPath);
        return {
          ipaPath: isolatedPath,
          ipaSize: size,
        };
      }
  ...
  try {
    for (const matchedPath of matchedPaths) {
      ...
      if (await isAppBundle(fullPath)) {
        ...
        try {
          supportedPlatforms = await this.appInfosCache.extractAppPlatforms(fullPath);
        } catch (e) {
          this.log.info(e.message);
          continue;
        }
      ...
      } else if (this.isRealDevice() && _.endsWith(_.toLower(fullPath), IPA_EXT) && (await fs.stat(fullPath)).isFile()) {
        this.log.debug(`Selecting the application at '${fullPath}'`);
        try {
          await this.appInfosCache.put(fullPath);
        } catch (e) {
          this.log.info(e.message);
          continue;
        }
        ...
        await fs.mv(fullPath, isolatedPath, {mkdirp: true});
        return isolatedPath;
      }
    }
  } finally {
...
export async function onPostConfigureApp({cachedAppInfo, isUrl, appPath}) {
  ...
  const isLocalApp = await isAppBundle(/** @type {string} */(appPath));
  const isLocalIpa = (await fs.stat(/** @type {string} */(appPath))).isFile()
    && /** @type {string} */(appPath).toLowerCase().endsWith(IPA_EXT);
  ...
  // Only local .app bundles that are available in-place
