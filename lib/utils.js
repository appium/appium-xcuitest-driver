...
async function getPidUsingAppName (pgrepPattern) {
...
async function killAppUsingAppName (pgrepPattern) {
  for (const signal of [2, 15, 9]) {
    if (!await getPidUsingAppName(pgrepPattern)) {
      return;
    }
...
async function resetXCTestProcesses (udid, isSimulator) {
  ...
  for (const [, pgrepPattern] of processMapping) {
    await killAppUsingAppName(pgrepPattern);
  }
}
...
export { detectUdid, getAndCheckXcodeVersion, getAndCheckIosSdkVersion,
         ...
         DEFAULT_TIMEOUT_KEY, resetXCTestProcesses };
