...
/**
 * Return true if the deviceType is tv
 * @param {string} deviceType The name of the device type
 * @returns {boolean} Return true if the deviceType is tv
 */
function isTvDevice (deviceType) {
  return _.toLower(deviceType) === _.toLower(DEVICE_TYPE_TV);
}

/**
 * Return the OS name of the device type
 * @param {string} deviceType The name of the device type
 * @returns {boolean} Return the OS name of the device type
 */
function getDeviceOSType (deviceType) {
  if (_.toLower(deviceType) === _.toLower(DEVICE_TYPE_TV)) {
    return DEVICE_OS_TVOS;
  }
...
export { detectUdid, getAndCheckXcodeVersion, getAndCheckIosSdkVersion,
  ...
  verifyApplicationPlatform, isTvDevice, getDeviceOSType, DEVICE_OS_IOS, DEVICE_OS_TVOS };
