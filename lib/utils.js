...
function parseKnownArgs (allArgs, knownArgNames) {
  return _.toPairs(allArgs).reduce((acc, [argName, argValue]) => {
    if (knownArgNames.includes(argName)) {
      ...
      const knownArgs = Object.keys(ARGS_CONSTRAINTS);
...
function parseArgs (opts, allArgs, knownArgNames) {
  ...
  acc = validateCaps(acc, ARGS_CONSTRAINTS);
  for (let arg in ARGS_CONSTRAINTS) {
    if (!_.has(acc, arg)) {
      if (_.has(opts, arg)) {
        acc[arg] = opts[arg];
      }
...
export {
  ...
  isLocalHost, normalizePlatformVersion, clearLogs, parseArgs
};
