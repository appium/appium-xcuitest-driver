...
import { validateCaps } from 'appium-base-driver/build/lib/basedriver/capabilities';
...
/*
const SERVER_ARGS = {
  'webkitDebugProxyPort': {isNumber: true, presence: true, default: 27753},
  'wdaLocalPort': {isNumber: true, presence: true, default: 8100},
};
*/

const CONSTRAINTS = {
  webkitDebugProxyPort: {isNumber: true, presence: true},
  wdaLocalPort: {isNumber: true, presence: true},
};
...
function parseKnownArgs (opts, allArgs, knownArgNames) {
  return _.toPairs(allArgs).reduce((acc, [argName, argValue]) => {
    if (knownArgNames.includes(argName)) {
      acc[argName] = argValue;
    } else if (_.has(opts, argName)) {
      acc[argName] = opts[argName];
    } else {
      throw new Error(`"${argName}" is not a recognized key`);
    }
    return acc;
  }, {});
}

function parseArgs (opts, allArgs, knownArgNames) {
  let acc = parseKnownArgs(opts, allArgs, knownArgNames);
  _.keys(DEFAULT_SERVER_ARGS).forEach((arg) => {
    if (!_.has(acc, arg)) {
      if (_.has(opts, arg)) {
        acc[arg] = opts[arg];
      } else {
        acc[arg] = DEFAULT_SERVER_ARGS[arg];
      }
    }
  });
  acc = validateCaps(acc, CONSTRAINTS);
...
export {
  ...
  isLocalHost, normalizePlatformVersion, clearLogs, parseArgs, DEFAULT_SERVER_ARGS
};
