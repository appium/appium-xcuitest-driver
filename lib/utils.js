...
function parseKnownArgs (allArgs, knownArgNames) {
  return _.toPairs(allArgs).reduce((acc, [argName, argValue]) => {
    ...
    const errMsg = `"${argName}" ${argValue} is not of type ${SERVER_ARGS[argName].type}`;

    // specifically check for 'int'
    if (SERVER_ARGS[argName].type === 'int') {
      if (!_.isInteger(argValue)) {
        throw new Error(errMsg);
      }
    } else if (!(typeof argValue === SERVER_ARGS[argName].type)) {
      throw new Error(errMsg);
    }
...
function parseArgs (opts, allArgs, knownArgNames) {
  let acc = parseKnownArgs(allArgs, knownArgNames);
  _.keys(SERVER_ARGS).forEach((key) => {
    if (knownArgNames.includes(key)) {
      if (!_.has(acc, key)) {
        if (_.has(opts, key)) {
          acc[key] = opts[key];
        } else {
          acc[key] = SERVER_ARGS[key].default;
        }
      }
    }
  });
...
export {
  ...
  isLocalHost, normalizePlatformVersion, clearLogs, parseArgs, SERVER_ARGS
};
