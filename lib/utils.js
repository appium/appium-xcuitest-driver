...
function translateDeviceName (platformVersion, devName = '') {
  ...
  switch (devName.toLowerCase().trim()) {
    ...
    case 'ipad simulator':
      ...
      deviceName = compareVersions(platformVersion, '<', '10.3') ? 'iPad Retina' : 'iPad Air';
      break;
  }
 ...
 * ==, !=, >, <, <=, >=, =
 * @returns {?boolean} null if any of the given arguments is not valid. Otherwise true or false
 * depending on the actual comparison result
 */
function compareVersions (ver1, operator, ver2) {
  ver1 = semver.valid(semver.coerce(`${ver1}`));
  if (!ver1) {
    log.info(`'${ver1}' version cannot be parsed and thus compared`);
    return null;
  }
  ver2 = semver.valid(semver.coerce(`${ver2}`));
  if (!ver2) {
    log.info(`'${ver2}' version cannot be parsed and thus compared`);
    return null;
  }
  if (!['==', '!=', '>', '<', '>=', '<=', '='].includes(operator)) {
    log.warn(`Versions comparison operator '${operator}' is not supported`);
    return null;
  }

  const semverOperator = ['==', '!='].includes(operator) ? '=' : operator;
  const result = semver.satisfies(ver1, `${semverOperator}${ver2}`);
...
export { detectUdid, getAndCheckXcodeVersion, getAndCheckIosSdkVersion,
  ...
  verifyApplicationPlatform, compareVersions };
