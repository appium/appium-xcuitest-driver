...
import { validateCaps } from 'appium-base-driver';
...
function parseKnownArgs (allArgs, knownArgNames) {
  return _.toPairs(allArgs).reduce((acc, [argName, argValue]) => {
    if (knownArgNames.includes(argName)) {
      ...
      const knownArgs = Object.keys(DEFAULT_SERVER_ARGS);
...
function parseArgs (opts, allArgs, knownArgNames) {
  ...
  for (let arg in DEFAULT_SERVER_ARGS) {
    if (!_.has(acc, arg)) {
      acc[arg] = _.has(opts, arg) ? opts[arg] : DEFAULT_SERVER_ARGS[arg];
    }
  }
