...
import { WEBKIT_DEBUG_PROXY_PORT, WEBDRIVERAGENT_PORT } from './driver';
...
function parseWebkitDebugProxyPort (driverArgs) {
  if (_.isEmpty(driverArgs)) {
    return WEBKIT_DEBUG_PROXY_PORT;
  }
  if (!_.has(driverArgs, 'webkit-debug-proxy-port')) {
    return WEBKIT_DEBUG_PROXY_PORT;
  }

  const webkitDebugProxyPort = driverArgs['webkit-debug-proxy-port'];

  if (!_.isInteger(webkitDebugProxyPort)) {
    throw new Error(`value of key 'webkit-debug-proxy-port' is not an integer`);
  }
  return webkitDebugProxyPort;
}

function parseWdaLocalPort (driverArgs) {
  if (_.isEmpty(driverArgs)) {
    return WEBDRIVERAGENT_PORT;
  }
  if (!_.has(driverArgs, 'webdriveragent-port')) {
    return WEBDRIVERAGENT_PORT;
  }

  const wdaLocalPort = driverArgs['webdriveragent-port'];

  if (!_.isInteger(wdaLocalPort)) {
    throw new Error(`value of key 'webdriveragent-port' is not an integer`);
  }
  return wdaLocalPort;
}
...
export {
  ...
  isLocalHost, normalizePlatformVersion, clearLogs, parseWebkitDebugProxyPort, parseWdaLocalPort
};
