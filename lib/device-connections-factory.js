...
class iProxy {
  ...
  async start () {
    ...
    this.localServer = net.createServer(async (localSocket) => {
      let remoteSocket;
      try {
        // 'Connection was refused to port 8100' error loop never finish
        // when we called this outside createServer even if the port had been available.
  ...
  _closeLocalServer () {
    ...
    this.log.debug(`Closing the connection to ${this.udid}`);
