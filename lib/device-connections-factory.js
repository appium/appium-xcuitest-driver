...
class DeviceConnectionsFactory {
  ...
  listConnections (udid = null, port = null, strict = false) {
    ...
    return _.keys(this._connectionsMapping)
      .filter((key) => strict
        ? key.startsWith(expectedKey) && key.endsWith(expectedKey)
        : key.startsWith(expectedKey) || key.endsWith(expectedKey));
  }

  async requestConnection (udid, port, options = {}) {
    if (!udid || !port) {
      log.warn('Either device UDID or local port is unset. ' +
        'Did not know how to request the connection');
      return;
    }
    ...
    if (usePortForwarding) {
      ...
      if (isPortBusy && !_.isEmpty(connectionsOnPort)) {
        ...
        for (const key of connectionsOnPort.filter(filterFunc)) {
