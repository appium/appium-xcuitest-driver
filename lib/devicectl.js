...
import {util, tempDir, fs} from 'appium/support';
...
export class Devicectl {
  ...
  async execute(subcommand, opts) {
    ...
    if (jsonOutput) {
      finalArgs.push('--json-output', jsonOutput);
    }
  ...
  async listProcesses() {
    ...
    const jsonOutput = path.join(tmpRoot, 'processes.json');
    try {
      await this.execute(['device', 'info', 'processes'], {
        jsonOutput,
      });
      return JSON.parse(await fs.readFile(jsonOutput, 'utf8')).result.runningProcesses;
    } finally {
   ...
   * @returns {Promise<Apps[]>} Empty array is returned if no matching apps are found
   */
  async listApps(bundleId = null) {
    const tmpRoot = await tempDir.openDir();
    const jsonOutput = path.join(tmpRoot, 'apps.json');
    ...
    try {
      ...
      return JSON.parse(await fs.readFile(jsonOutput, 'utf8')).result.apps;
    } finally {
      await fs.rimraf(tmpRoot);
    }
  }
