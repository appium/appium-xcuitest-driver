...
class IOSDeploy {
  ...
  async installApplication (bundlePathOnPhone) {
    ...
    try {
      ...
      try {
        await appInstalledNotification.timeout(APPLICATION_NOTIFICATION_TIMEOUT);
      } catch (e) {
    ...
    try {
      ...
      await fs.walkDir(app, true, async (itemPath, isDir) => {
        ...
        if (isDir) {
          ...
          promises.push(new B((resolve, reject) => {
      ...
      try {
        await B.all(promises).timeout(APPLICATION_PUSH_TIMEOUT).each(p => {
          if (p.isRejected()) {
            throw p.reason();
          }
        });
      } catch (e) {
        throw new Error(`Couldn't push all the files within the timeout ${APPLICATION_PUSH_TIMEOUT}ms. Error: ${e.message}`);
      }
