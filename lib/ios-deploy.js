...
class IOSDeploy {
  ...
  async pushAppBundle (app, opts = {}) {
    ...
    log.debug(`Successfully scanned the tree structure of ${path.basename(app)}`);
    ...
    try {
      ...
      for (const relativeFolderPath of foldersToPush.sort((a, b) => a.split(path.sep).length - b.split(path.sep).length)) {
        // createDirectory does not accept folder names ending with a slash
        const absoluteFolderPath = _.trimEnd(path.join(bundlePathOnPhone, relativeFolderPath), '/');
