...
class IOSDeploy {
    ...
    const installWithIosDeploy = async () => {
      ...
      try {
        await exec(IOS_DEPLOY, [
          '--id', this.udid,
          '--bundle', app,
        ], {timeout: timeout ?? DEFAULT_ITEM_PUSH_TIMEOUT});
      } catch (err) {
  ...
  async pushAppBundle (app, opts = {}) {
    const {
      timeoutMs = DEFAULT_ITEM_PUSH_TIMEOUT,
    ...
    const timer = new timing.Timer().start();
    const itemsToPush = await fs.glob('**', {cwd: app});
    const [foldersToPush, filesToPush] = itemsToPush
      .reduce((acc, x) => {
        acc[_.endsWith(x, '/') ? 0 : 1].push(x);
        return acc;
      }, [[], []]);
    ...
    try {
      ...
      for (const relativeFolderPath of foldersToPush) {
        await afcService.createDirectory(path.join(bundlePathOnPhone, relativeFolderPath));
      }
      log.debug(`Successfully created the remote folder structure (${util.pluralize('item', foldersToPush.length, true)})`);
      log.debug('Proceeding to files push');

      const pushFile = async (relativePath) => {
        const absolutePath = path.join(app, relativePath);
        const readStream = fs.createReadStream(absolutePath, {autoClose: true});
        const writeStream = await afcService.createWriteStream(path.join(bundlePathOnPhone, relativePath), {
          autoDestroy: true
        });
        ...
        await filePushPromise.timeout(timeoutMs,
          `Could not push '${absolutePath}' within the timeout of ${timeoutMs}ms. ` +
          `Consider increasing the value of 'appPushTimeout' capability.`);
      };

      if (enableParallelPush) {
        const chunks = _.chunk(_.shuffle(filesToPush), MAX_PUSH_CHUNK_SIZE);
        log.debug(`Got ${util.pluralize('files chunk', chunks.length, true)} to push. ` +
          `The maximum size of a single chunk is ${MAX_PUSH_CHUNK_SIZE}`);
        for (const [idx, chunk] in _.zip(_.range(chunks.length), chunks)) {
          const pushPromises = [];
          for (const relativeFilePath of chunk) {
            pushPromises.push(pushFile(relativeFilePath));
          }
          await B.all(pushPromises);
          log.debug(`Finished pushing files chunk #${idx + 1} of ${chunks.length}`);
        }
      } else {
        for (const fPath of filesToPush) {
          await pushFile(fPath);
        }
      }
      ...
      log.debug(`Successfully pushed ${util.pluralize('app folder', foldersToPush.length, true)} ` +
        ...
        `after ${timer.getDuration().asMilliSeconds.toFixed(0)}ms`);
