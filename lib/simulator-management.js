...
const SAFARI_OPTS_ALIASES_MAP = {
  safariAllowPopups: ['WebKitJavaScriptCanOpenWindowsAutomatically', 'JavaScriptCanOpenWindowsAutomatically'],
  safariIgnoreFraudWarning: ['WarnAboutFraudulentWebsites'],
  safariOpenLinksInBackground: ['OpenLinksInBackground'],
};
...
async function setSafariPrefs (sim, opts = {}) {
  ...
  for (const [name, aliases] of _.toPairs(SAFARI_OPTS_ALIASES_MAP)) {
    if (!_.has(opts, name)) {
      continue;
    }

    for (const alias of aliases) {
      safariSettings[alias] = Number(Boolean(opts[name]));
    }
  }
...
async function setLocalizationPrefs (sim, opts = {}) {
  ...
  const i10nConfig = {};
  if (language) {
    i10nConfig.language = {name: language};
  }
  if (locale) {
    i10nConfig.locale = {name: locale};
    if (calendarFormat) {
      i10nConfig.locale.calendar = calendarFormat;
    }
  }
  if (_.isEmpty(i10nConfig)) {
    return false;
  }

  log.debug(`About to update internationalization preferences: ${JSON.stringify(i10nConfig)}`);
  await sim.configureLocalization(i10nConfig);
  return true;
}
