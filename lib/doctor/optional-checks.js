...
export class OptionalSimulatorCheck {
  async diagnose() {
    const errorPrefix = 'Testing on Simulator is not possible';
    try {
      // https://github.com/appium/appium/issues/12093#issuecomment-459358120
      await exec('xcrun', ['simctl', 'help']);
    } catch (err) {
      return doctor.nokOptional(
        `${errorPrefix}. Cannot run 'xcrun simctl': ${err.stderr || err.message}`
      );
    }

    const sdks = await this._listInstalledSdks();
    const hasIosSdk = sdks.some(({platform}) => platform === 'iphonesimulator');
    if (!hasIosSdk) {
      return doctor.nokOptional(`${errorPrefix}. No iOS Simulator SDKs are installed`);
    }

    return doctor.okOptional(
      `The following Simulator SDKs are installed:\n` +
      sdks.map(({displayName}) => `\t${displayName}`).join('\n')
    );
  }

  async fix() {
    return `Install the desired Simulator SDK from Xcode's Preferences -> Components`;
  }
