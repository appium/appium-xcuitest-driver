...
import {pushFile, IO_TIMEOUT_MS} from './ios-fs-helpers';
...
export class RealDevice {
   ...
   */
  async install(ipaPath, bundleId, opts = {}) {
    const {
    ...
    try {
      ...
      const bundlePathOnPhone = `/${path.basename(ipaPath)}`;
      await pushFile(afcService, ipaPath, bundlePathOnPhone, {
        timeoutMs,
      });
    ...
    this.log.info(`The of installation '${bundleId}' succeeded after ${timer.getDuration().asMilliSeconds.toFixed(0)}ms`);
  }
   ...
   * @param {InstallOptions} [opts={}]
   */
  async installApp(ipaPath, bundleId, opts = {}) {
    return await this.install(ipaPath, bundleId, opts);
  }
