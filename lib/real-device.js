  ...
  async install(app, timeout, strategy = null) {
    ...
    if (_.toLower(/** @type {'ios-deploy'} */ (strategy)) === APP_INSTALL_STRATEGY.IOS_DEPLOY) {
      ...
      try {
        ...
        await B.resolve(pushFolder(afcService, app, bundlePathOnPhone, {enableParallelPush})).timeout(timeout);
        ...
        if (err instanceof B.TimeoutError) {
          ...
          if (!enableParallelPush) {
            this.log.info(
              `Consider setting the value of 'appInstallStrategy' capability to 'parallel' (currently '${strategy ?? 'serial'}')`
            );
          }
        }
