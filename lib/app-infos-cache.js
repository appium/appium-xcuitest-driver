  ...
  async _putIpa(ipaPath) {
    ...
    try {
      await zip.readEntries(ipaPath, async ({entry, extractEntryTo}) => {
        // For furutre reference:
        // If the directory included `.app` (case insensitive) name as part of directory name
        // like 'Payload/something.App.app/filename', this 'entry.fileName' returns 'Payload/something.App/filename'.
        // This looks like this is macOS dependent behavior. Technically such a naming could exist, but
        // the session will get no valid plist error.
