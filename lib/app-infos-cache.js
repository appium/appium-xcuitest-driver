...
export class AppInfosCache {
   ...
   */
  async put (appPath) {
    return (await fs.stat(appPath)).isFile()
      ? await this._putIpa(appPath)
      : await this._putApp(appPath);
  }
      ...
      await zip.readEntries(ipaPath, async ({entry, extractEntryTo}) => {
        ...
        try {
          ...
          if (entry.uncompressedSize <= MAX_MANIFEST_SIZE) {
            const payload = await this._readPlist(plistPath, ipaPath);
            if (_.isPlainObject(payload)) {
              plistCandidates[hash] = [entry.fileName, payload];
            }
          }
        } catch (e) {
   ...
   * @param {string} appPath Full path to .ipa or .app bundle
   * @returns {Promise<any>} The payload of the plist file
   */
  async _readPlist(plistPath, appPath) {
    try {
