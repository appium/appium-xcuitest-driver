      ...
      try {
        await zip.readEntries(appPath, async ({entry, extractEntryTo}) => {
          ...
          if (PLIST_CACHE.has(hash)) {
            plistPayload = PLIST_CACHE.get(hash);
            return false;
          }
          ...
          try {
            ...
            plistPayload = await readPlist(plistPath);
            if (entry.uncompressedSize <= MAX_PLIST_SIZE) {
              this.log.debug(
                `Caching the manifest for ${payload?.CFBundleIdentifier} from an archived source using the key '${hash}'`
              );
              PLIST_CACHE.set(hash, plistPayload);
            }
          } catch (e) {
            this.log.debug(e.stack);
            lastError = e;
