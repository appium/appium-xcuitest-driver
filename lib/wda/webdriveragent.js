...
class WebDriverAgent {
   ...
   * Or reuse it if it is default id without updatedWDABundleId.
   * Uninstall it if the method faces except for the above situation.
   *
   * @param {string} updatedWDABundleId BundleId you'd like to use
   * @return {undefined} Return undefined since the method calls uninstall sets this.webDriverAgentUrl as void.
   */
  async setupCaching (updatedWDABundleId) {
    ...
    if (status && status.build) {
      ...
      log.debug(`WDA is currently not running`);
      return;
    }
