...
class WebDriverAgent {
   ...
   * 'com.customised.WDA.xctrunner' for real device if a user sets 'updatedWDABundleId' as 'com.customised.WDA'.
   *
  ...
  async uninstall (updatedWDABundleId = WDA_RUNNER_BUNDLE_ID) {
    try {
      if (this.xcodeVersion.major < 11) {
        // Can remove without errors
        log.debug(`Removing WDA application from device: '${WDA_BUNDLE_ID}'`);
        ...
        log.debug(`Removing WDA application from device: '${bundle_id}'`);
        await this.device.removeApp(bundle_id);
      }
    } catch (e) {
  ...
  async quitAndUninstall (updatedWDABundleId = WDA_RUNNER_BUNDLE_ID) {
    ...
    if (status && status.build) {
      ...
      if (util.hasValue(productBundleIdentifier)) {
        updatedWDABundleId = productBundleIdentifier;
      }
    }
