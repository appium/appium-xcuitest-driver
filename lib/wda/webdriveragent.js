...
import { checkForDependencies, WDA_RUNNER_BUNDLE_ID, getWDACommitHash } from './utils';
...
class WebDriverAgent {
  ...
  async setupCaching (updatedWDABundleId) {
    ...
    const actualCommitHash = await getWDACommitHash();
    log.debug(`Commit hash of the bundled WDA: ${actualCommitHash}`);
    if (actualCommitHash && commitHash && _.toLower(actualCommitHash) !== _.toLower(commitHash)) {
      log.info('Will uninstall running WDA since it has different version in comparison to the one ' +
        'which is bundled with appium-xcuitest-driver module');
      return await this.uninstall();
    }
