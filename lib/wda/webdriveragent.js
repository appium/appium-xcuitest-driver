...
class WebDriverAgent {
  ...
  async setupCaching (updatedWDABundleId) {
    ...
    log.debug(`Upgrade timestamp of the WDA on the device: ${upgraded_at}`);
    if (actualUpgradeTimestamp && upgraded_at && _.toLower(`${actualUpgradeTimestamp}`) !== _.toLower(`${upgraded_at}`)) {
      log.info('Will uninstall running WDA since it has different version in comparison to the one ' +
        `which is bundled with appium-xcuitest-driver module (${actualUpgradeTimestamp} != ${upgraded_at})`);
      return await this.uninstall();
    }
