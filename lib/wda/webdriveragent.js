...
import {checkForDependencies, WDA_RUNNER_BUNDLE_ID} from './utils';
...
class WebDriverAgent {
  ...
  async getRunningBundleId () {
    ...
    if (status && status.build) {
      try {
        return status.build.productBundleIdentifier;
      } catch (e) {
        log.warn(`Can't get productBundleIdentifier. Please update WebDriverAgent.`);
      }
    }
    return null;
  }
  ...
  /**
   * Uninstall WDA which running in the test device, if the bundle is isn't default or updatedWDABundleId.
   *
   * @param {string} updatedWDABundleId BundleId you'd like to set
   * @return {boolean} True if WDA is running
   * @throws {Error} If there was invalid response code or body
   */
  async uninstallWDAIfRunningBundleIdIsDifferent (updatedWDABundleId) {
    // Old WDA has no bundleId in the /status. Then we only can obtain `null` as the currentAppBundleId.
    // Thus, `isRunning()` is the only way to confirm if WDA is running in the target device.
    const currentAppBundleID = await this.getRunningBundleId();
    if (util.hasValue(currentAppBundleID) && util.hasValue(updatedWDABundleId) && updatedWDABundleId !== currentAppBundleID) {
      log.info(`Will uninstall running WDA since it has different bundle id. The actual value is '${currentAppBundleID}'.`);
      await this.uninstall();
    } else if (util.hasValue(currentAppBundleID) && !util.hasValue(updatedWDABundleId) && WDA_RUNNER_BUNDLE_ID !== currentAppBundleID) {
      log.info(`Will uninstall running WDA since it isn't default, '${WDA_RUNNER_BUNDLE_ID}', bundle id.`);
      await this.uninstall();
    } else {
      const message = util.hasValue(currentAppBundleID)
          ? `Will reuse previously cached WDA instance at '${this.url.href}' with '${currentAppBundleID}'`
