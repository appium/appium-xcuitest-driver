...
const COMMIT_HASH_STORE = 'wda.hash';
...
/**
 * Retrieves WDA revision hash
 *
 * @return {?string} The hash string or null if the hash store file cannot be found
 */
async function getWDACommitHash () {
  const moduleRoot = findRoot(__dirname);
  if (await fs.exists(path.resolve(moduleRoot, COMMIT_HASH_STORE))) {
    return (await fs.readFile(path.resolve(moduleRoot, COMMIT_HASH_STORE), 'utf8')).trim();
  }
  return null;
}
