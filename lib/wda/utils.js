...
async function killAppUsingAppName (udid, appName) {
  // false result indicates that no processes were killed
  const execPkill = async (signal) => {
    const args = [signal, '-if', `${appName}.*${udid}`];
    try {
  ...
  if (await execPkill('-2')) {
    return;
  }
  let tryNum = 0;
  while (tryNum < 3) {
    if (!await getPidUsingAppName(udid, appName)) {
      return;
    }
    await B.delay(300);
    if (await execPkill('-15')) {
      return;
    }
    ++tryNum;
  }
