...
async function setXctestrunFile (deviceInfo, sdkVersion, bootstrapPath, wdaRemotePort) {
  ...
  const updateWDAPort = getWDAPort(deviceInfo.platformName, wdaRemotePort);
 ...
 */
function getWDAPort (platformName, wdaRemotePort) {
  const runner = `WebDriverAgentRunner${_.toLower(platformName) === _.toLower(PLATFORM_NAME_TVOS) ? '_tvOS' : ''}`;
...
function getXctestrunFileName (deviceInfo, version) {
  return _.toLower(deviceInfo.platformName) === _.toLower(PLATFORM_NAME_TVOS)
