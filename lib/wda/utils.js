 ...
 */
async function setXctestrunFile (isRealDevice, udid, platformVersion, sdkVersion, bootstrapPath, wdaRemotePort) {
  const xctestrunFilePath = await getXctestrunFilePath(isRealDevice, udid, platformVersion, sdkVersion, bootstrapPath);
...
async function getXctestrunFilePath (isRealDevice, udid, platformVersion, sdkVersion, bootstrapPath) {
  // For newer Xcode (Xcode 10, at least)
  const xctestrunDeviceFileNameSDK = `${udid}_${sdkVersion}.xctestrun`;
  const xctestrunDeviceFilePathSDK = path.resolve(bootstrapPath, xctestrunDeviceFileNameSDK);

  // For compatibility. Can remove below when we drop Xcode 9
  // Xcode 9.4 worked with sdk version, but not sure under Xcode 9.3 also worked with sdk version based
  const xctestrunDeviceFileNamePlatform = `${udid}_${platformVersion}.xctestrun`;
  const xctestrunDeviceFilePathPlatform = path.resolve(bootstrapPath, xctestrunDeviceFileNamePlatform);

  let originalXctestrunFile;

  // Return sdk version based path prior than platform based path
  if (await fs.exists(xctestrunDeviceFilePathSDK)) {
    return xctestrunDeviceFilePathSDK;
  }
  originalXctestrunFile = path.resolve(bootstrapPath, _getXctestrunFilePathName(isRealDevice, sdkVersion));
  if (await fs.exists(originalXctestrunFile)) {
    ...
    await fs.copyFile(originalXctestrunFile, xctestrunDeviceFilePathSDK);
    return xctestrunDeviceFilePathSDK;
  }

  if (await fs.exists(xctestrunDeviceFilePathPlatform)) {
    return xctestrunDeviceFilePathPlatform;
  }
  originalXctestrunFile = path.resolve(bootstrapPath, _getXctestrunFilePathName(isRealDevice, platformVersion));
  if (await fs.exists(originalXctestrunFile)) {
    // If this is first time run for given device, then first generate xctestrun file for device.
    // We need to have a xctestrun file **per device** because we cant not have same wda port for all devices.
    await fs.copyFile(originalXctestrunFile, xctestrunDeviceFilePathPlatform);
    return xctestrunDeviceFilePathPlatform;
  }

  log.errorAndThrow(`if you are using useXctestrunFile capability then you need to have ${
...
function _getXctestrunFilePathName (isRealDevice, version) {
  return isRealDevice
    ? `WebDriverAgentRunner_iphoneos${version}-arm64.xctestrun`
    : `WebDriverAgentRunner_iphonesimulator${version}-x86_64.xctestrun`;
}
