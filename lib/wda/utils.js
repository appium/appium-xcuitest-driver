...
const FBAPPIUMACTIONSSYTHESIZER_FILE = 'WebDriverAgentLib/Utilities/FBAppiumActionsSynthesizer.m';
const FBW3CACTIONSSYNTHESIZER_FILE = 'WebDriverAgentLib/Utilities/FBW3CActionsSynthesizer.m';
const XCUIAPPLICATION_FBTOUCHACTION_FILE = 'WebDriverAgentLib/Categories/XCUIApplication+FBTouchAction.m';

async function replaceInFile (file, find, replace, initial = true) {
...
async function fixFBAppiumActionsSynthesizerFile (bootstrapPath, initial = true) {
  const file = path.resolve(bootstrapPath, FBAPPIUMACTIONSSYTHESIZER_FILE);

  let oldDef = '#import "XCTRunnerDaemonSession.h"';
  let newDef = '// #import "XCTRunnerDaemonSession.h"';
  await replaceInFile(file, oldDef, newDef, initial);

  oldDef = 'if (![XCTRunnerDaemonSession sharedSession].useLegacyEventCoordinateTransformationPath) {';
  newDef = 'if (false) { // FBAPPIUMACTIONSSYTHESIZER_FILE';
  await replaceInFile(file, oldDef, newDef, initial);
}

async function fixFBW3CActionsSynthesizerFile (bootstrapPath, initial = true) {
  const file = path.resolve(bootstrapPath, FBW3CACTIONSSYNTHESIZER_FILE);
  ...
  oldDef = 'if (![XCTRunnerDaemonSession sharedSession].useLegacyEventCoordinateTransformationPath) {';
  newDef = 'if (false) { // FBW3CACTIONSSYNTHESIZER_FILE';
  await replaceInFile(file, oldDef, newDef, initial);
}

async function fixXCUIApplicationFBTouchActionFile (bootstrapPath, initial = true) {
  const file = path.resolve(bootstrapPath, XCUIAPPLICATION_FBTOUCHACTION_FILE);

  let oldDef = '#import "XCTRunnerDaemonSession.h"';
  let newDef = '// #import "XCTRunnerDaemonSession.h"';
  await replaceInFile(file, oldDef, newDef, initial);

  oldDef = '  __block BOOL didSucceed;\n' +
           ...
           '  }];\n' +
           '  return didSucceed;';
  newDef = '  return true; // XCUIAPPLICATION_FBTOUCHACTION_FILE';
  await replaceInFile(file, oldDef, newDef, initial);
}

async function fixForXcode82 (bootstrapPath, initial = true, fixXcode9 = true) {
  if (fixXcode9) {
    await fixForXcode9(bootstrapPath, !initial, false);
  }
  await fixFBAppiumActionsSynthesizerFile(bootstrapPath, initial);
  await fixFBW3CActionsSynthesizerFile(bootstrapPath, initial);
  await fixXCUIApplicationFBTouchActionFile(bootstrapPath, initial);
}
...
async function fixFBSessionFile (bootstrapPath, initial = true) {
  ...
  let oldDef = 'return [FBApplication fb_activeApplication] ?: self.testedApplication;';
  let newDef = 'FBApplication *application = [FBApplication fb_activeApplication] ?: self.testedApplication;\n' +
                '  return application;';
  await replaceInFile(file, oldDef, newDef, initial);
}

async function fixForXcode8 (bootstrapPath, initial = true, fixXcode9 = true) {
  if (fixXcode9) {
    await fixForXcode9(bootstrapPath, !initial, false);
  }
...
async function fixForXcode9 (bootstrapPath, initial = true, fixXcode8 = true) {
  if (fixXcode8) {
    await fixForXcode8(bootstrapPath, !initial, false);
  }
...
export { updateProjectFile, resetProjectFile, checkForDependencies,
         setRealDeviceSecurity, fixForXcode8, fixForXcode82, fixForXcode9,
