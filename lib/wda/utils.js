 ...
 * @property {string} platformNameLowerCase - The platform name of iOS, tvOS
*/
/**
...
async function setXctestrunFile (deviceInfo, sdkVersion, bootstrapPath, wdaRemotePort) {
  ...
  const updateWDAPort = getWDAPort(deviceInfo.platformNameLowerCase, wdaRemotePort);
...
/**
 * Return the WDA object which has USER_PORT
 * @param {DeviceInfo}
 * @param {string} version - The Xcode SDK version of OS.
 * @return {object} returns an runner object which has USE_PORT
 */
function getWDAPort (platformNameLowerCase, wdaRemotePort) {
  const runner = platformNameLowerCase === 'tvos'
...
async function getXctestrunFilePath (deviceInfo, sdkVersion, bootstrapPath) {
  ...
  for (const [filePath, version] of [sdkBased, platformBased]) {
    ...
    const originalXctestrunFile = path.resolve(bootstrapPath, getXctestrunFilePathName(deviceInfo, version));
  ...
  log.errorAndThrow(`If you are using 'useXctestrunFile' capability then you ` +
                                  ...
                                  `'${path.resolve(bootstrapPath, getXctestrunFilePathName(deviceInfo, sdkVersion))}')`);
}
 ...
 * @return {string} returns xctestrunFilePath for given device
 */
function getXctestrunFilePathName (deviceInfo, version) {
  return deviceInfo.platformNameLowerCase === 'tvos'
...
export { updateProjectFile, resetProjectFile, checkForDependencies,
  setRealDeviceSecurity, fixForXcode7, fixForXcode9, getWDAPort,
  getXctestrunFilePathName, generateXcodeConfigFile, setXctestrunFile,
