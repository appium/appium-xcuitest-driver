...
async function getXctestrunFilePath (isRealDevice, udid, platformVersion, sdkVersion, bootstrapPath) {
  ...
  originalXctestrunFile = path.resolve(bootstrapPath, getXctestrunFilePathName(isRealDevice, sdkVersion));
  ...
  originalXctestrunFile = path.resolve(bootstrapPath, getXctestrunFilePathName(isRealDevice, platformVersion));
  if (await fs.exists(originalXctestrunFile)) {
    // If this is first time run for given device, then first generate xctestrun file for device.
    // We need to have a xctestrun file **per device** because we cant not have same wda port for all devices.
  ...
  log.errorAndThrow(`if you are using useXctestrunFile capability then you need to have ${
    path.resolve(bootstrapPath, getXctestrunFilePathName(isRealDevice, sdkVersion))} file`);
}
