 ...
 * @property {string} deviceType - The device type such as tv, iphone or ipad
*/
/**
...
async function setXctestrunFile (deviceInfo, sdkVersion, bootstrapPath, wdaRemotePort) {
  ...
  const updateWDAPort = getAdditionalRunContent(deviceInfo.deviceType, wdaRemotePort);
 ...
 */
function getAdditionalRunContent (deviceType, wdaRemotePort) {
  const runner = `WebDriverAgentRunner${isTvDevice(deviceType) ? '_tvOS' : ''}`;
...
function getXctestrunFileName (deviceInfo, version) {
  return isTvDevice(deviceInfo.deviceType)
