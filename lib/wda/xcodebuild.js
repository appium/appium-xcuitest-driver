...
class XcodeBuild {
  ...
  async retrieveDerivedDataPath () {
    ...
    if (!match) {
      ...
      const logSize = (await fs.stat(logFile)).size;
      // https://github.com/nodejs/node/issues/3175
      if (logSize >= process.binding('buffer').kMaxStringLength / 2) {
        log.warn(`Cannot read the xcodebuild log file '${logFile}', ` +
                 `because it is too big (${logSize / 1000000.0} MB). ` +
                 `Try to clean up the Xcode derived data folder to address the problem.`);
        return;
      }
