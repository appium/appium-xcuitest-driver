...
import { fixForXcode8, fixForXcode82, fixForXcode9, setRealDeviceSecurity, generateXcodeConfigFile,
...
class XcodeBuild {
  ...
  async init (noSessionProxy) {
    ...
    if (this.xcodeVersion.major === 8 && this.xcodeVersion.minor === 0) {
      log.debug(`Using Xcode ${this.xcodeVersion.versionString}, so fixing WDA codebase for Xcode 8.0`);
      await fixForXcode8(this.bootstrapPath, true);
    }

    if (this.xcodeVersion.major === 8 && this.xcodeVersion.minor <= 2) {
      log.debug(`Using Xcode ${this.xcodeVersion.versionString}, so fixing WDA codebase for Xcode <= 8.2`);
      await fixForXcode82(this.bootstrapPath, true);
    }

    if (this.xcodeVersion.major === 9) {
      log.debug(`Using Xcode ${this.xcodeVersion.versionString}, so fixing WDA codebase for Xcode 9`);
