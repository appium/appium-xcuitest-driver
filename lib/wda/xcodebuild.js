...
class XcodeBuild {
  ...
  async init (noSessionProxy) {
    ...
    if (!this.isFirstSession) {
      log.debug('Not touching any project files, because the current session is not the first one in the pipeline');
      return;
    }
  ...
  async reset () {
    if (!this.isFirstSession) {
      log.debug('Skipping project files reset, because the current session is not the first one in the pipeline');
      return;
    }
