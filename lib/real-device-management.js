  ...
  try {
    ...
    if (!shouldUninstallApp && e.message.includes('MismatchedApplicationIdentifierEntitlement')) {
      log.info(`The application identified by '{bundleId}' cannot be installed because it might ` +
        `be already cached on the device, probably with a different signature by some reasons. ` +
    ...
    if (shouldUninstallApp) {
      ...
      log.info(`An error occurred during installing '${app}'. '${bundleId}' might remain ` +
