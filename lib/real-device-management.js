...
async function installToRealDevice(device, app, bundleId, opts) {
  ...
  try {
    ...
    // TODO: check some cases.
    // This case is only when the app installation is failed as different signature.
    log.info(`The application ${bundleId} might be remained as a cache or offload with different application identifier. ` +
      'Going to uninstall the application and retrying to install the given application');
