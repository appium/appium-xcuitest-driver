...
import { escapeRegExp, find, filter, isString, isObject } from 'lodash';
...
function parseAttr (cssAttr) {
  ...
  const attrName = toSnakeCase(assertGetAttrName(cssAttr));
  ...
  if (BOOLEAN_ATTRS.includes(attrName)) {
    return `${attrName} == ${assertGetBool(cssAttr)}`;
  }
...
function parseCssRule (cssRule) {
  ...
  const nonIndexAttrs = filter(attrs, (attr) => isString(attr));
  if (nonIndexAttrs && nonIndexAttrs.length > 0) {
    iosClassChainSelector += `[\`${nonIndexAttrs.join(' AND ')}\`]`;
  }

  const indexAttr = find(attrs, (attr) => isObject(attr) && attr.index);
  if (indexAttr) {
    iosClassChainSelector += `[${indexAttr.index}]`;
  }

  if (cssRule.rule) {
    if (cssRule.rule.nestingOperator === '>') {
