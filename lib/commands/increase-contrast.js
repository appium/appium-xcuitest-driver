...
export default {
  ...
  async mobileSetIncreaseContrast(increaseContrast) {
    ...
    if (!INCREASE_CONTRAST_CONFIG.includes(_.lowerCase(increaseContrast))) {
      throw new errors.InvalidArgumentError(`The 'increaseContrast' value is expected to be one of ${INCREASE_CONTRAST_CONFIG.join(',')}`);
    }
  ...
  async mobileGetIncreaseContrast() {
    assertSimulator(this);
    return /** @type {IncreaseContrast} */ (await /** @type {import('../driver').Simulator} */ (this.device).getIncreaseContrast());
  },
};
