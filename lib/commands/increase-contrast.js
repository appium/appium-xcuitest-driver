...
export default {
  ...
  async mobileSetIncreaseContrast(increaseContrast) {
    ...
    if (!INCREASE_CONTRAST_CONFIG.includes(increaseContrast)) {
      throw new errors.InvalidArgumentError(`The 'increaseContrast' value is expected to be one of ${INCREASE_CONTRAST_CONFIG.join(',')} (string, case-sensitive)`);
    }

    await /** @type {import('../driver').Simulator} */ (this.device).setIncreaseContrast(increaseContrast);
  },
   ...
   *
   * @since Xcode 15 (but lower xcode could have this command)
   * @returns {Promise<IncreaseContrast>} the contrast configuration value.
  ...
  async mobileGetIncreaseContrast() {
    assertSimulator(this);
    return await /** @type {import('../driver').Simulator} */ (this.device).getIncreaseContrast();
  },
};
 ...
 * @typedef {import('./types').IncreaseContrast} IncreaseContrast
 */
