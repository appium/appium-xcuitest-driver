...
extensions.onPageChange = async function onPageChange (pageChangeNotification) {
  ...
  function isUrlBlacklisted (url, safariIgnoreWebHostnames) {
    const blacklistedHosts = (safariIgnoreWebHostnames || '')
      .split(',')
      .map((b) => b.trim())
      .filter((b) => !_.isEmpty(b));
    for (const blacklistedHost of blacklistedHosts) {
      if (blacklistedHost === 'about:blank' && url === 'about:blank') {
        return true;
      } else {
        try {
          const hostname = new URL(url).hostname;
          if (hostname === blacklistedHost) {
            return true;
          }
        } catch (ign) {
  ...
  for (const page of pageArray) {
    ...
    if (!_.includes(this.contexts, contextId)) {
      if (isUrlBlacklisted(page.url, this.opts.safariIgnoreWebHostnames)) {
