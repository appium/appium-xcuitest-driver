...
commands.closeApp = async function () {
  try {
    ...
    await request({
          ...
          url: url.format(nativeDriver.url) + 'deactivateApp',
          method: 'POST',
          resolveWithFullResponse: true
        })
        .then(function (response) {
          // Deactivate succeeded...
          logger.info(`Successfully deactivated the app. Response: ${response}`);
        })
        .catch(function (err) {
          // Deactivate failed...
          logger.warn(`Something went wrong whilst deactivating the app: ${err}`);
        });
