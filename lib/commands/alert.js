...
helpers.nativeSetAlertText = async function (text) {
  try {
    ...
    await this.findNativeElementOrElements('class name', 'XCUIElementTypeAlert', false);
  } catch (err) {
    log.debug(`Error finding alert element: ${err.message}`);
    throw new errors.NoAlertOpenError();
  }

  let possibleTextFields = await this.findElOrEls('class chain', "**/XCUIElementTypeAlert/**/XCUIElementTypeAny[`type == 'XCUIElementTypeTextField' OR type == 'XCUIElementTypeSecureTextField'`]", true);
