...
helpers.nativeSetAlertText = async function (text) {
  ...
  try {
    ...
    alert = await this.findNativeElementOrElements('class name', 'XCUIElementTypeAlert', false);
    alert = util.unwrapElement(alert);
  } catch (err) {
    log.debug(`Error finding alert element: ${err.message}`);
    throw new errors.NoAlertOpenError();
  }

  let possibleTextFields = await this.findElOrEls('-ios predicate string', `type == 'XCUIElementTypeTextField' OR type == 'XCUIElementTypeSecureTextField'`, true, alert);
  if (possibleTextFields.length !== 1) {
    let msg = `${possibleTextFields.length ? 'More than one' : 'No'} text ` +
              `field or secure text field found. Unable to type into the alert`;
