...
helpers.nativeSetAlertText = async function (text) {
  let alert = await this.findNativeElementOrElements('class name', 'XCUIElementTypeAlert', true);
  if (alert.length !== 1) {
    throw new errors.NoAlertOpenError();
  }
  alert = util.unwrapElement(alert[0]);
  let possibleTextFields = await this.findNativeElementOrElements('class name', 'XCUIElementTypeTextField', true, alert);
  if (!possibleTextFields.length) {
    possibleTextFields = await this.findNativeElementOrElements('class name', 'XCUIElementTypeSecureTextField', true, alert);
  }
  if (!possibleTextFields.length) {
    throw new Error(`No text field or secure text field found. Unable to type into the alert`);
  } else if (possibleTextFields.length > 1) {
    throw new Error(`More than one text field or secure text field found. Unable to type into the alert`);
  }

  await this.setValue(text, util.unwrapElement(possibleTextFields[0]));
};
...
commands.setAlertText = async function (text) {
  ...
  try {
    ...
    if (!this.isWebContext()) {
      await this.nativeSetAlertText(text);
      return;
    }
