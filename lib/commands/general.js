...
commands.background = async function (duration) {
  ...
  if (_.isUndefined(seconds)) {
    log.errorAndThrow(`commands.background: Argument value is expected to be an object or 'undefined'.
     ...
     The 'timeout' attribute can be 'null' or any negative number to put the app under test into background and never come back
     or a positive number of milliseconds to wait until the app is restored.`);
  }
  let params = {};
  if (seconds) {
