...
commands.hideKeyboard = async function (strategy, ...possibleKeys) {
  ...
  if (possibleKeys.length) {
    ...
    if (!await this.getAttribute('visible', keyboard)) {
...
commands.getDevicePixelRatio = iosCommands.device.getDevicePixelRatio;
commands.getViewportRect = iosCommands.device.getViewportRect;

commands.getScreenHeight = async function () {
  return (await this.getWindowSize()).height;
};

commands.getStatusBarHeight = async function () {
  try {
    let statusBar = await this.findNativeElementOrElements(`class name`, `XCUIElementTypeStatusBar`, false);
    return (await this.getSize(statusBar)).height;
  } catch (e) {
    log.warn(`Could not detect status bar, assuming it is of zero height. ` +
