...
commands.mobileStartPerfRecord = async function mobileStartPerfRecord (opts = {}) {
  ...
  const cwd = (await fs.exists('/Applications/Utilities'))
    ? '/Applications/Utilities' : undefined;
  const proc = new SubProcess(instrumentsPath, args, { cwd });
