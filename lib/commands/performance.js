 ...
 *
 * @property {?string} traceLogPath - The file path to the save the .trace file in local.
...
commands.mobileStopPerfRecord = async function (opts = {}) {
  ...
  if (!_.isEmpty(traceLogPath)) {
    log.info(`Found perf trace record '${localPath}'. Move to path '${traceLogPath}'.`);
    return await fs.rename(localPath, traceLogPath);
  }
