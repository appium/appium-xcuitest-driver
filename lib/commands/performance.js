...
class PerfRecorder {
  constructor (reportPath, udid, opts = {}) {
    ...
    this._logger = logger.getLogger(
      `${_.truncate(this.profileName, {length: 10})}@${this.udid.substring(0, 8)}`);
  ...
  async getZippedReportPath () {
    ...
    if (!this._archivePromise) {
      this._archivePromise = B.resolve(zip.toArchive(zippedReportPath, {
  ...
  async _enforceTermination () {
    ...
    if (await fs.exists(this._reportPath)) {
  ...
  async start () {
    ...
    this._archivePromise = null;
    this._logger.debug(`Starting ${INSTRUMENTS_BINARY}: ${quote(fullCmd)}`);
    ...
    try {
      await waitForCondition(async () => !!(await this.getOriginalReportPath()), {
        waitMs: STARTUP_TIMEOUT_MS,
        intervalMs: 500,
      });
    } catch (e) {
      await this._enforceTermination();
      this._logger.errorAndThrow(`There is no .trace file found for performance profile '${this._profileName}' ` +
        `after ${STARTUP_TIMEOUT_MS}ms. Make sure the profile is supported on this device. ` +
        `You can use 'instruments -s' command to see the list of all available profiles. ` +
        `Check the server log for more details`);
    }
    this._logger.info(`The performance recording has started. ` +
      `Will timeout in ${util.pluralize('second', this._timeLimit, true)}`);
  }
  ...
  async stop (force = false) {
    ...
    return new B((resolve, reject) => {
      this._process.once('exit', async (code, signal) => {
        if (code === 0) {
          resolve(await this.getZippedReportPath());
        } else {
          reject(new Error(`Performance recording exited with error code ${code}, signal ${signal}`));
        }
      });

      try {
        this._process.stop('SIGINT', STOP_TIMEOUT_MS);
      } catch (e) {
        reject(new Error(`Performance recording has failed to exit after ${STOP_TIMEOUT_MS}ms`));
      }
    });
  }
}
...
commands.mobileStartPerfRecord = async function mobileStartPerfRecord (opts = {}) {
  ...
  const reportPath = path.resolve(os.tmpdir(),
    `appium_perf_${profileName.replace(/\W/g, '_')}_${UUID.create().hex.substring(0, 8)}.${DEFAULT_EXT}`);
  let realPid;
  if (pid) {
...
commands.mobileStopPerfRecord = async function mobileStopPerfRecord (opts = {}) {
  ...
  return await encodeBase64OrUpload(resultPath, remotePath, { user, pass, method });
};
