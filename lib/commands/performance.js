 ...
 *
 * @property {?string} traceLogPath - The file path to the save the zipped .trace file in local.
 ...
 *                                  Null or empty string value for traceLogPath and remotePath (the default setting) means the content of resulting
 ...
 * directly as base64-encoded zip archive or uploaded to a remote location or save to a local location
...
commands.mobileStopPerfRecord = async function (opts = {}) {
  ...
  const {traceLogPath, remotePath, user, pass, method, profileName=DEFAULT_PROFILE_NAME} = opts;
  const runningRecorders = RECORDERS_CACHE[profileName];
  if (!_.isPlainObject(runningRecorders) || !runningRecorders[this.opts.device.udid]) {
  ...
  try {
    ...
    if (!_.isEmpty(traceLogPath)) {
      log.info(`Found zipped perf trace record '${zipPath}'. Move to path '${traceLogPath}'.`);
      await fs.rename(zipPath, traceLogPath);
      return '';
    }
