...
class PerfRecorder {
  ...
  async getOriginalReportPath () {
    const resultPaths = await fs.glob(`*${DEFAULT_EXT}`, {
      cwd: this._reportRoot,
      absolute: true,
      strict: false,
    });
    return _.first(resultPaths) || '';
  }
  ...
  async _enforceTermination () {
    ...
    await fs.rimraf(this._reportRoot);
    return '';
  }
