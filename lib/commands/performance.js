 ...
 *
 * @property {?string} savePath - The file path to the save the .trace file in local.
 ...
 *                                  Null or empty string value for savePath and remotePath (the default setting) means the content of resulting
...
commands.mobileStopPerfRecord = async function (opts = {}) {
  ...
  const {savePath, remotePath, user, pass, method, profileName=DEFAULT_PROFILE_NAME} = opts;
  const runningRecorders = RECORDERS_CACHE[profileName];
  if (!_.isPlainObject(runningRecorders) || !runningRecorders[this.opts.device.udid]) {
  ...
  if (!_.isEmpty(savePath)) {
    log.info(`Found perf trace record '${localPath}'. Move to path '${savePath}'.`);
    return await fs.rename(localPath, savePath);
  }
