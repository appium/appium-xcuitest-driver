...
class SimulatorScreenRecorder extends ScreenRecorder {
  ...
  async start (timeout) {
    ...
    log.info(`Beginning screen recording on Simulator with command: '${this.binaryName} ${args.join(' ')}'. ` +
...
class RealDeviceScreenRecorder extends ScreenRecorder {
  ...
  async start (timeout) {
    ...
    try {
      ...
      log.errorAndThrow(`'${this.ffmpegBinary}' binary is not found in PATH. Install it via 'brew install ffmpeg'` +
        `Check https://www.ffmpeg.org/download.htmlfor more details.`);
    }
    ...
    log.info(`Beginning real device screen recording with command: '${this.binaryName} ${args.join(' ')}'`);
    await this.setupBroadcastCapture(port);
    log.info(`Will timeout in ${timeout / 1000} s`);
