...
class ScreenRecorder {
  ...
  async startIproxy (localPort) {
    ...
    try {
      ...
      log.warn(`Cannot start iproxy. Assuming it is already forwarding the remote port ${this.opts.remotePort} to ${localPort} ` +
        `for the device ${this.udid}. Original error: ${err.message}`);
      this.iproxy = null;
    }
  }
  ...
  async terminate () {
    ...
    if (this.mainProcess && this.mainProcess.isRunning) {
      const terminatePromise = this.mainProcess.stop('SIGTERM');
      this.mainProcess = null;
      try {
  ...
  async interrupt () {
    ...
    if (this.mainProcess && this.mainProcess.isRunning) {
      const interruptPromise = this.mainProcess.stop('SIGINT');
      this.mainProcess = null;
      try {
