  ...
  ws.on('message', async (message) => {
    ...
    if (isHeaderMessageReceived) {
      ...
      if (previousFrameTimestamp) {
        const webpBuffer = await sharp(Buffer.from(frameData))
          .toFormat(sharp.format.webp)
          .toBuffer();
        encoder.add(`data:image/webp;base64,${webpBuffer.toString('base64')}`,
          hrtimeDiffToMs(process.hrtime(previousFrameTimestamp)));
      }
      previousFrameTimestamp = process.hrtime();
    } else {
...
commands.startRecordingScreen = async function (options = {}) {
  ...
  if (this.isRealDevice()) {
    ...
    if (QUALITY_LEVELS[_.toLower(videoQuality)]) {
      args.push('--resolution', QUALITY_LEVELS[_.toLower(videoQuality)]);
    } else {
      log.errorAndThrow(`Unknown videoQuality value '${videoQuality}'. ` +
        `Supported values are: ${JSON.stringify(_.keys(QUALITY_LEVELS))}`);
    }
  } else {
