...
class SimulatorScreenRecorder extends ScreenRecorder {
  ...
  async start (timeout) {
    ...
    this.mainProcess.on('output', (stdout, stderr) => {
      if (stderr) {
        log.debug(`[${this.binaryName}] ${stderr}`);
      }
    });
  ...
  async terminate () {
    ...
    if (!this.mainProcess) {
      const pid = await getPidUsingPattern(`simctl io ${this.udid} recordVideo`);
      if (!pid) {
        return true;
      }
...
class RealDeviceScreenRecorder extends ScreenRecorder {
  ...
  async start (timeout) {
    try {
      ...
      throw new Error(`'${this.binaryName}' binary is not found in PATH. Install it using 'brew install ffmpeg'` +
    ...
    const args = [
      ...
      '-i', `http://localhost:${this.iproxy.localport}`,
      '-vcodec', 'mjpeg',
      '-y', this.videoPath,
    ];
    this.mainProcess = new SubProcess(this.binaryName, args);
    this.mainProcess.on('output', (stdout, stderr) => {
      if (stderr) {
        log.warn(`[${this.binaryName}] ${stderr}`);
      }
    });
    await this.mainProcess.start(0);
    log.info(`Starting screen capture on Real device with command: '${this.binaryName} ${args.join(' ')}'. ` +
      `Will timeout in ${timeout}ms`);
  ...
  async startIproxy () {
    const localPort = await findAPortNotInUse(DEFAULT_MJPEG_SERVER_PORT, DEFAULT_MJPEG_SERVER_PORT + 100);
    log.debug(`Will use the local port #${localPort} and the remote port #${this.opts.remotePort} `+
      `for the device '${this.udid}'`);
