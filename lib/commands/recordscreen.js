...
const DEFAULT_RESOLUTION = '400x600';
...
class SimulatorScreenRecorder extends ScreenRecorder {
  ...
  async terminate () {
    ...
    if (this.mainProcess && this.mainProcess.isRunning) {
      try {
        await this.mainProcess.stop('SIGKILL');
      } catch (e) {
...
class RealDeviceScreenRecorder extends ScreenRecorder {
  ...
  videoQualityToFps () {
    if (!util.hasValue(this.opts.videoQuality)) {
      return DEFAULT_FPS;
    }

    switch (_.toLower(this.opts.videoQuality)) {
      case 'low':
        return 10;
      case 'medium':
        return DEFAULT_FPS;
      ...
      case 'photo':
        return 25;
      default:
        throw new Error(`The current videoQuality value '${this.opts.videoQuality}' is not supported`);
    }
  }
  ...
  async start (timeout) {
    try {
      ...
      throw new Error(`'${this.binaryName}' binary is not found in PATH. ` +
  ...
  async setupBroadcastCapture (port) {
    const minicapStream = new PassThrough();
    let ffmpegStream = this.ffmpegProcess.proc.stdin;
    minicapStream.pipe(ffmpegStream, {end:false});
    ...
    return new B((resolve, reject) => {
      (function poll (retry = 0) {
        ...
        function onBannerAvailable (banner) {
          ...
          log.debug(`Virtual screen WxH: ${banner.virtualWidth}x${banner.virtualHeight}`);
        ...
        function onFrameAvailable (frame) {
          resolve();
          if (ffmpegStream) {
            minicapStream.write(frame.buffer);
            ++totalFramesCount;
          }
          // fs.writeFile(`/Users/elf/Desktop/pics/pic_${totalFramesCount}.jpg`, frame.buffer);
        }
        ...
        stream.on('close', () => {
          ...
          if (totalFramesCount) {
            ffmpegStream.end();
            ffmpegStream = null;
            log.info(`Recorded ${totalFramesCount} video frames in total`);
          }
        });
      })();
    });
  ...
  async terminate () {
    ...
    if (this.ffmpegProcess && this.ffmpegProcess.isRunning) {
      try {
        try {
          this.ffmpegProcess.proc.stdin.end();
        } catch (ign) {}
        if (this.ffmpegProcess.isRunning) {
          await this.ffmpegProcess.stop('SIGKILL');
        }
      } catch (e) {
        log.warn(`Cannot terminate ${this.ffmpegBinary}. Original error: ${e.message}`);
        result = false;
      }
    }
  ...
  async interrupt () {
    ...
    if (this.ffmpegProcess && this.ffmpegProcess.isRunning) {
      try {
        ...
        if (this.ffmpegProcess.isRunning) {
          await this.ffmpegProcess.stop('SIGHUP', 60000);
        }
      } catch (e) {
        log.warn(`Cannot interrupt ${this.ffmpegBinary}. Original error: ${e.message}`);
        result = false;
      }
    }
