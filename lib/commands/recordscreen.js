 ...
 *                                  and `forceRestart` paramater is not set to `true`.
...
commands.startRecordingScreen = async function (options = {}) {
  const {remotePath, user, pass, method, videoType, forceRestart} = options;
  let result = '';
  ...
  if (!_.isEmpty(this._recentScreenRecordingPath)) {
    try {
      if (!forceRestart) {
        result = await uploadRecordedMedia(this._recentScreenRecordingPath, remotePath, {user, pass, method});
      }
    } catch (err) {
...
commands.stopRecordingScreen = async function (options = {}) {
  ...
  if (_.isEmpty(pid)) {
    ...
    if (!_.isEmpty(this._recentScreenRecordingPath)) {
      result = await uploadRecordedMedia(this._recentScreenRecordingPath, remotePath, {user, pass, method}, keepRecordedMedia);
      this._recentScreenRecordingPath = null;
    }
    return result;
  }
