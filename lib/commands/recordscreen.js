  ...
  async start (timeout) {
    ...
    this.timeoutHandler = setTimeout(async () => {
      if (!await this.interrupt()) {
        log.warn(`Cannot finish the active screen recording on the device '${this.udid}' after ${timeout}ms timeout`);
      }
    }, timeout);
  }
  ...
  async terminate () {
    ...
    if (this.mainProcess && this.mainProcess.isRunning) {
      try {
        ...
        log.warn(`Cannot terminate ${FFMPEG_BINARY}. Original error: ${e.message}`);
        result = false;
      }
      this.mainProcess = null;
    }
