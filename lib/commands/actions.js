...
async function pushFileToSimulator (device, remotePath, base64Data) {
  if (remotePath.startsWith(CONTAINER_PATH_MARKER)) {
    if (!remotePath.includes('/')) {
      log.errorAndThrow(`It is expected that bundle identifier is separated from the relative path with a single slash. ` +
                        `'${remotePath}' is given instead`);
    }
    const bundleId = remotePath.substring(CONTAINER_PATH_MARKER.length, remotePath.indexOf('/'));
    const containerRoot = await getAppContainer(device.udid, bundleId);
    const dstPath = path.resolve(containerRoot, remotePath.substring(remotePath.indexOf('/') + 1));
    log.info(`Parsed bundle identifier '${bundleId}' from '${remotePath}'. ` +
             `Will put the data info '${dstPath}'`);
...
async function pushFileToRealDevice (device, remotePath, base64Data) {
  ...
  try {
    ...
    if (remotePath.startsWith(CONTAINER_PATH_MARKER)) {
      ...
      const bundleId = remotePath.substring(CONTAINER_PATH_MARKER.length, remotePath.indexOf('/'));
      dstPath = path.resolve(mntRoot, remotePath.substring(remotePath.indexOf('/') + 1));
      log.info(`Parsed bundle identifier '${bundleId}' from '${remotePath}'. ` +
               `Will put the data info '${dstPath}'`);
    ...
    try {
      if (!await fs.exists(path.dirname(dstPath))) {
        log.debug(`The destination folder ${path.dirname(dstPath)} does not exist on the device. Creating...`);
        await fs.mkdirp(path.dirname(dstPath));
      }
      await fs.writeFile(dstPath, new Buffer(base64Data, 'base64').toString('binary'), 'binary');
    } finally {
