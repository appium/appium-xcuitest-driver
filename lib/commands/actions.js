...
commands.mobileSimctl = async function (opts = {}) {
  if (!this.relaxedSecurityEnabled) {
    log.errorAndThrow(`Appium server must have relaxed security flag set in order to run 'mobile:simctl' command`);
  }

  let {args} = extractMandatoryOptions(opts, ['args']);
  if (!_.isArray(args)) {
    args = [args];
  }
  const timeout = _.isNumber(opts.timeout) ? parseInt(opts.timeout, 10) : 2 * 60 * 1000;
  const {stdout} = await exec('xcrun', ['simctl', ...args], {timeout});
  return stdout;
};
