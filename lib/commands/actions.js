...
function verifyBundleIdPresence (opts) {
  if (!_.isString(opts.bundleId) || !opts.bundleId.trim()) {
    log.errorAndThrow(`bundleId is expected to be set to a valid value. '${opts.bundleId}' is passed instead.`);
  }
}

commands.mobileLaunchApp = async function (opts = {}) {
  verifyBundleIdPresence(opts);
  return await this.proxyCommand('/session/apps/launch', 'POST', {bundleId: opts.bundleId});
};

commands.mobileTerminateApp = async function (opts = {}) {
  verifyBundleIdPresence(opts);
  return await this.proxyCommand('/session/apps/terminate', 'POST', {bundleId: opts.bundleId});
};

commands.mobileActivateApp = async function (opts = {}) {
  verifyBundleIdPresence(opts);
  return await this.proxyCommand('/session/apps/activate', 'POST', {bundleId: opts.bundleId});
};

commands.mobileQueryAppState = async function (opts = {}) {
  verifyBundleIdPresence(opts);
  return await this.proxyCommand('/session/apps/state', 'POST', {bundleId: opts.bundleId});
};
