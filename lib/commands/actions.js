...
function verifyOptions (opts) {
  if (!_.isString(opts.bundleId) || !opts.bundleId.trim()) {
    log.errorAndThrow(`bundleId is expected to be set to a valid value.` +
                      `'${opts.bundleId}' is given instead`);
  }
  return {bundleId: opts.bundleId};
}

commands.mobileInstallApp = async function (opts = {}) {
  const {app} = opts;
  if (_.isEmpty(app)) {
    log.errorAndThrow(`'app' parameter is mandatory. '${app}' is given instead.`);
  }

  let dstPath = app;
  let isTemporary = false;
  if (dstPath.toLowerCase().startsWith('http') && dstPath.includes('://')) {
    const appExt = '.app';
    let isCompressed = false;
    try {
      // download if necessary
      log.info(`Application address '${dstPath}' seems to be an URL. Downloading...`);
      const {targetPath, contentType} = await this.helpers.downloadApp(dstPath, appExt);
      isCompressed = _.includes(ZIP_EXTS, path.extname(dstPath).toLowerCase()) || contentType === ZIP_MIME_TYPE;
      dstPath = targetPath;
      isTemporary = true;
    } catch (err) {
      log.errorAndThrow(`Bad app: '${dstPath}'. App paths need to be absolute or relative to the Appium ` +
        'server install dir, or an URL to a compressed file.');
    }
    if (isCompressed) {
      log.info(`Downloaded application '${dstPath}' seems to be a compressed file. Extracting...`);
      const archivePath = dstPath;
      try {
        dstPath = await this.helpers.unzipApp(archivePath, appExt);
      } finally {
        await fs.unlink(archivePath);
      }
      if (path.extname(dstPath) !== appExt) {
        log.errorAndThrow(`The extracted app path '${dstPath}' did not have extension '${appExt}'`);
      }
    }
  }
...
commands.mobileIsAppInstalled = async function (opts = {}) {
  ...
  if (_.isEmpty(bundleId)) {
    log.errorAndThrow(`'bundleId' parameter is mandatory. '${bundleId}' is given instead.`);
  }

  return await this.opts.device.isAppInstalled(bundleId);
};
...
commands.mobileLaunchApp = async function (opts = {}) {
  return await this.proxyCommand('/wda/apps/launch', 'POST', verifyOptions(opts));
};

commands.mobileTerminateApp = async function (opts = {}) {
  return await this.proxyCommand('/wda/apps/terminate', 'POST', verifyOptions(opts));
};

commands.mobileActivateApp = async function (opts = {}) {
  return await this.proxyCommand('/wda/apps/activate', 'POST', verifyOptions(opts));
};

commands.mobileQueryAppState = async function (opts = {}) {
  return await this.proxyCommand('/wda/apps/state', 'POST', verifyOptions(opts));
};
