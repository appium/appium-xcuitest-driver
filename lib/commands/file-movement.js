...
async function createAfcClient(udid, bundleId, containerType) {
  ...
  const {
    skipDocumentsContainerCheckInFileManagement = false,
  } = await this.settings.getSettings();

  return skipDocumentsContainerCheckInFileManagement
    ? service.vendContainer(bundleId)
    : isDocumentsContainer(containerType)
