...
function isDocuments (containerType) {
  return containerType.toLowerCase() === IFUSE_CONTAINER_DOCUMENTS;
}
...
async function createService (udid, remotePath) {
  if (CONTAINER_PATH_PATTERN.test(remotePath)) {
    ...
    const relativePath = isDocuments(containerType) ? path.resolve(CONTAINER_DOCUMENTS_PATH, pathInContainer) : pathInContainer;
  ...
  try {
    ...
    if (CONTAINER_PATH_PATTERN.test(remotePath)) {
      ...
      ifuseArgs = [
        '-u',
        device.udid,
        isDocuments(containerType) ? '--documents' : '--container',
        bundleId,
        mntRoot
      ];
    } else {
