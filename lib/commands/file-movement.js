 ...
 */
async function pullFromSimulator (isFile, device, remotePath) {
  if (!remotePath.startsWith(CONTAINER_PATH_MARKER)) {
    log.errorAndThrow(`Only pulling ${isFile ? 'files' : 'folders'} from application containers is supported for iOS Simulator. ` +
                      `Provide the remote path in format @<bundle_id>/<path_to_the_${isFile ? 'file' : 'folder'}_in_its_container>`);
  }
  const [bundleId, dstPath] = await parseContainerPath(remotePath,
    async (x) => await getAppContainer(device.udid, x));
  log.info(`Parsed bundle identifier '${bundleId}' from '${remotePath}'. ` +
 ...
 */
async function pullFromRealDevice (isFile, device, remotePath) {
  await verifyIFusePresence();
  const mntRoot = await tempDir.openDir();
  let isUnmountSuccessful = true;
  try {
...
commands.pullFile = async function (remotePath) {
  ...
  return this.isSimulator()
    ? await pullFromSimulator(true, this.opts.device, remotePath)
    : await pullFromRealDevice(true, this.opts.device, remotePath);
};
...
commands.pullFolder = async function (remotePath) {
  ...
  return this.isSimulator()
    ? await pullFromSimulator(false, this.opts.device, remotePath)
    : await pullFromRealDevice(false, this.opts.device, remotePath);
};
