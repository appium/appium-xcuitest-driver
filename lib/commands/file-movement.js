...
async function pullFileFromRealDevice (service, relativePath) {
  ...
  try {
    ...
    throw new Error(`Couldn't pull the file within the given timeout ${IO_TIMEOUT}ms`);
  }
...
async function pullFolderFromRealDevice (service, relativePath) {
  ...
  try {
    ...
    try {
      ...
      throw new Error(`Couldn't pull all items in the folder within the given timeout ${IO_TIMEOUT}ms`);
    }
...
async function pushFileToRealDevice (device, remotePath, base64Data) {
  ...
  try {
    ...
    try {
      ...
      throw new Error(`Couldn't push the file within the given timeout ${IO_TIMEOUT}ms`);
    }
  } finally {
    service.close();
  }
