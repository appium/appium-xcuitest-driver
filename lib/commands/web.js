...
extensions.getExtraTranslateWebCoordsOffset = async function getExtraTranslateWebCoordsOffset (wvPos, realDims) {
  ...
  if (isScrolled) {
    ...
    // Safari on iOS 15+ has navigation bar in the bottom
    // TODO: allow to switch with Settings API since this "bar" is configuable
    // via Settings app.
    topOffset = tabBarPosition === TAB_BAR_POSITION_BOTTOM
      ? notchOffset
      : IPHONE_TOP_BAR_HEIGHT + notchOffset;
    log.info(`tabBarPosition and topOffset: ${tabBarPosition}, ${topOffset}`);
