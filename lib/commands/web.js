...
extensions.clickCoords = async function (coords) {
  ...
  // try to get the main application, to tap against
  let el = 'nil';
  try {
    el = await this.findNativeElementOrElements('class name', 'XCUIElementTypeApplication', false);
    el = util.unwrapElement(el);
  } catch (ign) {}

  // tap on absolute coordinates
  await this.proxyCommand(`/wda/tap/${el}`, 'POST', {x, y});
};
