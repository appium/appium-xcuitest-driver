...
extensions.getExtraTranslateWebCoordsOffset = async function getExtraTranslateWebCoordsOffset (wvPos, realDims) {
  ...
  const tabBarVisibility = nativeWebTapTabBarVisibility.toLowerCase();
  const bannerVisibility = nativeWebTapSmartAppBannerVisibility.toLowerCase();
  ...
  if (isScrolled) {
    ...
    if (orientation === 'LANDSCAPE' || !isIphone) {
      ...
      } else if (tabBarVisibility === DETECT || _.isNil(tabBarVisibility)) {
...
extensions.getExtraNativeWebTapOffset = async function getExtraNativeWebTapOffset (isIphone, bannerVisibility) {
  ...
  } else if (bannerVisibility === DETECT || _.isNil(bannerVisibility)) {
...
extensions.translateWebCoords = async function translateWebCoords (coords) {
  ...
  try {
    webview = await retryInterval(5, 100, async () => {
      const wv = await this.findNativeElementOrElements('class name', 'XCUIElementTypeWebView', false);
      if (!wv) {
    ...
    log.error(`Unable to find WebView.`);
    log.error(error);
  }
