...
const extensions = {
  ...
  async mobileCalibrateWebToRealCoordinatesTranslation() {
    ...
    await this.setUrl(`http://127.0.0.1:${this.opts.wdaLocalPort || 8100}/calibrate`);
    ...
    /** @type {import('../types').Delta} */
    try {
      this.webviewOffset = JSON.parse(await this.title());
    } catch (e) {
      throw new Error(
        `Cannot determine web view coordinates offset. Are you in Safari context? ` +
    ...
    return {
      // @ts-ignore webviewOffset is always defined here
      offset: this.webviewOffset
    };
  },
