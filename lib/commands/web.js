...
helpers.useAtomsElement = function useAtomsElement (el) {
  // can be ':wdc:1627938293483' by Safari
  const regEl = new RegExp(`\\d+`).exec(el);
  if (_.isNull(regEl)) {
    throw new errors.UnknownError(`Cannot convert element ID for using in WD atoms: '${el}'`);
  }

  let elementid = regEl[0];
  if (parseInt(elementid, 10) < ELEMENT_OFFSET) {
    log.debug(`Element with id '${elementid}' passed in for use with ` +
      `atoms, but it's out of our internal scope. Adding ${ELEMENT_OFFSET}.`);
    el = (parseInt(elementid, 10) + ELEMENT_OFFSET).toString();
  }
