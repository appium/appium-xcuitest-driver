...
extensions.getExtraNativeWebTapOffset = async function () {
  ...
  try {
    ...
    // this height is either there or not, so cache
    if (!util.hasValue(this._webTabBarHeight)) {
      el = util.unwrapElement(el);
      const height = (await this.getNativeRect(el)).height;
      log.debug(`Found tabs with height ${height}.`);
      this._webTabBarHeight = height;
    }
    offset = this._webTabBarHeight;
  } catch (ign) {
