...
extensions.getExtraTranslateWebCoordsOffset = async function () {
  ...
  try {
    ...
    iphone = useragent.toLowerCase().includes('iphone');
  } catch (err) {
...
extensions.getExtraNativeWebTapOffset = async function () {
  ...
  try {
    ...
    try {
      const el = await this.findNativeElementOrElements('xpath', `//XCUIElementTypeButton[@name='Close app download offer']/following-sibling::XCUIElementTypeOther[1]`, false);
      offset += await this.getElementHeightMemoized('SmartAppBanner', el);
    } catch (ign) {
