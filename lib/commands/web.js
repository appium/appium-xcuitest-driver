...
async function tapWebElementNatively (driver, atomsElement) {
  ...
  try {
    ...
    // in our CI we are getting weird failures, and need more info
    if (process.env.APPIUM_TRAVIS_CI) {
      const src = await driver.getNativePageSource();
      log.debug('Native source when failing to do native web tap:');
      log.debug(src);
    }
  }
  return false;
