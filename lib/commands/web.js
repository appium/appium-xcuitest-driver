...
extensions.getTabOffset = async function () {
  let offset = 0;
  let els = await this.findNativeElementOrElements('-ios predicate string', "wdName LIKE '*, Tab'", true);
  if (els.length) {
    els = (await B.all(els.map((el) => this.getNativeRect(el.ELEMENT)))).map((el) => el.height);
    log.debug(`Found ${els.length} apparent tabs with heights ${els.join(', ')}. Setting offset to ${els[0]}`);
    offset = els[0];
  }
  return offset;
};
