...
const extensions = {
  ...
  async translateWebCoords(x, y) {
    ...
    if (this.webviewCalibrationResult) {
      // it seems like the response of element position in Safari returns exact x and y,
      // so what here should do is just to add the offsetX/Y.
      // [debug] [RemoteDebugger] Received result for atom 'get_size' execution: {"width":61,"height":16}
      // [debug] [RemoteDebugger] Received data response from send (id: 86): '"{\"status\":0,\"value\":{\"x\":237.90499877929688,\"y\":623,\"ceil\":\"function(){this.x=Math.ceil(this.x);this.y=Math.ceil(this.y);return this}\",\"floor\":\"function(){this.x=Math.floor(this.x);...'
      this.log.debug(`Will use the recent calibration result: ${JSON.stringify(this.webviewCalibrationResult)}`);
      const { offsetX, offsetY } = this.webviewCalibrationResult;
      return {
        x: Math.trunc(offsetX + x),
        y: Math.trunc(offsetY + y),
      };
    } else {
      this.log.debug(
  ...
  async mobileCalibrateWebToRealCoordinatesTranslation() {
    ...
    // no page found page shows proper inner width/height.
    // Probably we could add the same something in the calibrate page to get current
    // inner width/height so that we can do the same logic in 'calibrate' page only
    await this.setUrl(`http://172.0.0.1:9999/nopage`);
    const innerSize = await this.getWindowRect();
    const browserWidth = innerSize.width;
    const browserHeigh = innerSize.height;
    ...
    await retryInterval(
      ...
      async () => {
        ...
        // @ts-ignore
        const pixelRatioX = browserWidth / width; // Safari does not have anything for 'width', so the ratio calculation can be X base.
        const pixelRatioY = browserHeigh / height; // Safari does not have anything for 'width', so the ratio calculation can be X base.
        this.webviewCalibrationResult = {
          offsetX: Math.trunc(centerX - x / pixelRatioX),
          offsetY: Math.trunc(centerY - y / pixelRatioY),
          // @ts-ignore
          pixelRatioX,
          pixelRatioY,
        };
