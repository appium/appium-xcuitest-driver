...
const extensions = {
  ...
  async translateWebCoords(x, y) {
    ...
    if (this.calibratedWebviewOffset) {
      this.log.debug(`Will use the previously calibrated offset: ${JSON.stringify(this.calibratedWebviewOffset)}`);
      return {
        x: x + this.calibratedWebviewOffset.dx,
        y: y + this.calibratedWebviewOffset.dy,
      };
    } else {
      this.log.debug(
  ...
  async mobileCalibrateWebToRealCoordinatesTranslation() {
    ...
    await retryInterval(
      ...
      async () => {
        ...
        this.calibratedWebviewOffset = {
          dx: centerX - x / pixelRatio,
          dy: centerY - y / pixelRatio,
        };
      }
    );
    ...
    // @ts-ignore calibratedWebviewOffset is always defined here
    return {offset: this.calibratedWebviewOffset};
  },
