...
extensions.getExtraTranslateWebCoordsOffset = async function getExtraTranslateWebCoordsOffset (wvPos, realDims) {
  ...
  if (isScrolled) {
    ...
    if (orientation === 'LANDSCAPE' || !isIphone) {
      // No need to check whether the tabs bar is visible or not
      // if already defined by nativeWebTapTabBarVisible in settings.
      const isTabBarVisible = settings.nativeWebTapTabBarVisible;

      if (isTabBarVisible) {
        topOffset += TAB_BAR_OFFSET;
      } else if (_.isNil(isTabBarVisible)) {
  ...
  // No need to check whether the Smart App Banner is visible or not
  // if already defined by nativeWebTapSmartAppBannerVisible in settings.
  const isBannerVisible = settings.nativeWebTapSmartAppBannerVisible;
  topOffset += await this.getExtraNativeWebTapOffset(isIphone, isBannerVisible);

  wvPos.y += topOffset;
  realDims.h -= (topOffset + bottomOffset);
};

extensions.getExtraNativeWebTapOffset = async function getExtraNativeWebTapOffset (isIphone, isAppBannerVisible) {
  let offset = 0;
  ...
  } else if (_.isNil(isAppBannerVisible)) {
...
extensions.translateWebCoords = async function translateWebCoords (coords) {
  ...
  let webview = await retryInterval(5, 100, async () => {
    const webview = await this.findNativeElementOrElements('class name', 'XCUIElementTypeWebView', false);
    if (!webview) {
