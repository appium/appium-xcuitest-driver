...
const helpers = {
  ...
  async executeAtom(atom, args, alwaysDefaultFrame = false) {
    ...
    if (
      ...
      return await this.waitForAtom(this.remote.executeAtom(atom, args, frames));
    }
    ...
    for (const _retryCount of _.range(RECONNECT_COUNT_ATOMS_IOS17)) {
      try {
        ...
        this.log.warn(`Retrying to send the same request after re-connecting to the Web Inspector.`);
