...
const CALIBRATION_TAP_DELTA = 5; // px
...
const extensions = {
  ...
  async translateWebCoords(x, y) {
    ...
    if (this.webviewCalibrationResult) {
      ...
      return {
        x: Math.trunc(offsetX + x * pixelRatioX),
        y: Math.trunc(offsetY + y * pixelRatioY),
      };
    } else {
      this.log.debug(
    ...
    await retryInterval(
      ...
      async () => {
        const {x: x0, y: y0} = await performCalibrationTap(
          centerX - CALIBRATION_TAP_DELTA, centerY - CALIBRATION_TAP_DELTA
        );
        ...
        this.webviewCalibrationResult = {
          offsetX: Math.trunc(centerX - CALIBRATION_TAP_DELTA - x0 * pixelRatioX),
          offsetY: Math.trunc(centerY - CALIBRATION_TAP_DELTA - y0 * pixelRatioY),
          pixelRatioX,
          pixelRatioY,
        };
      }
