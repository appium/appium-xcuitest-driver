...
helpers.cacheWebElement = function cacheWebElement (el, strict = true) {
...
helpers.cacheWebElementsIfPresent = function cacheWebElementsIfPresent (response) {
  if (_.isArray(response)) {
    return response.map(
      (item) => _.isArray(item) || _.isPlainObject(item)
        ? this.cacheWebElementsIfPresent(item)
        : this.cacheWebElement(item, false)
    );
  } else if (_.isPlainObject(response)) {
    return _.toPairs(response).reduce(
      (acc, [key, value]) => {
        acc[key] = _.isArray(value) || _.isPlainObject(value)
          ? this.cacheWebElementsIfPresent(value)
          : this.cacheWebElement(value, false);
        return acc;
      }, {}
    );
  }
...
helpers.cacheWebElements = function cacheWebElements (rawEl) {
  if (!(_.isArray(rawEl) || _.isPlainObject(rawEl))) {
    throw new errors.UnknownError(`Web element identifiers cannot be parsed from ` +
      JSON.stringify(rawEl));
  }
  return _.isArray(rawEl)
    ? rawEl.map((el) => this.cacheWebElement(el))
    : this.cacheWebElement(rawEl);
};
