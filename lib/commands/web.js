...
const RESPONSE_TIMEOUT = 3 * 60000;
...
extensions.checkForAlert = async function () {
  try {
    const alertText = await this.proxyCommand('/alert/text', 'GET');
    if (util.hasValue(alertText)) {
      log.debug(`Found an alert with text '${alertText}'`);
      return true;
    }
  } catch (ign) {}
  return false;
...
extensions.waitForAtom = async function (promise) {
  let isWaitingForAtom = true;
  const alertsHandler = async () => {
    while (isWaitingForAtom) {
      await B.delay(ALERT_CHECK_INTERVAL);
      if (!isWaitingForAtom) {
        return;
      }
      ...
      if (await this.checkForAlert()) {
        if (this.opts.autoAcceptAlerts) {
          log.debug('Accepting the alert as requested by autoAcceptAlerts capability');
          try {
            await this.proxyCommand('/alert/accept', 'POST', {});
          } catch (err) {
            log.warn(`Did not accept the alert. Original error: ${err.message}`);
          }
        } else if (this.opts.autoDismissAlerts) {
          log.debug('Dismissing the alert as requested by autoDismissAlerts capability');
          try {
            await this.proxyCommand('/alert/dismiss', 'POST', {});
          } catch (err) {
            log.warn(`Did not dismiss the alert. Original error: ${err.message}`);
          }
        } else {
          log.debug('Doing nothing. Set either autoAcceptAlerts or autoDismissAlerts capabilities ' +
            'if you want background alerts to be handled automatically in WEB context');
        }
        return;
      }
    }
  ...
  alertsHandler().catch((e) => log.warn(e));
  try {
    return this.parseExecuteResponse(await B.resolve(promise)
      .timeout(RESPONSE_TIMEOUT)
      .finally(() => {
        isWaitingForAtom = false;
      }));
  } catch (err) {
    if (err instanceof B.TimeoutError) {
      throw new Error(`Did not get any response after ${RESPONSE_TIMEOUT}s`);
    }
    throw err;
  }
};
