 ...
 */
async function toMobileConfig (certBuffer, commonName) {
...
async function trustCertificateInPreferences (driver, name) {
  await this.activateApp('com.apple.Preferences');
  await clickElement(driver, {
    type: '-ios predicate string',
    value: 'type == \'XCUIElementTypeButton\' AND label == \'General\''
  });
  await clickElement(driver, {
    type: 'accessibility id',
    value: 'About'
  ...
  await driver.mobileSwipe({
    element: await this.findNativeElementOrElements('class name', 'XCUIElementTypeTable', false),
    direction: 'up'
  });
  await clickElement(driver, {
    type: 'accessibility id',
...
commands.mobileInstallCertificate = async function (opts = {}) {
  ...
  const commonName = extractCommonName(certBuffer);
  const mobileConfig = await toMobileConfig(certBuffer, commonName);
  try {
  ...
  try {
    ...
    try {
      ...
      await trustCertificateInPreferences(this, commonName);
    } finally {
      try {
