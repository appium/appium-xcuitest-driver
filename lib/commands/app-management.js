...
import { installToSimulator } from '../simulator-management';
import { installToRealDevice } from '../real-device-management';
...
export default {
   ...
   * @param {boolean} [enforceAppInstall] - If the application installation follows currently installed application's version status if provided. No checking occurs if no this option.
   * @privateRemarks Link to capability docs
   * @returns {Promise<void>}
   * @this {XCUITestDriver}
   */
  async mobileInstallApp(app, timeoutMs, strategy, enforceAppInstall) {
    ...
    if (_.isUndefined(enforceAppInstall)) {
      // @ts-expect-error - do not assign arbitrary properties to `this.opts`
      ...
      this.log.debug(`enforceAppInstall is provided. Following the installation by checking the installed application status.`)
      const bundleId = await extractBundleId(srcAppPath);
      const {install, skipUninstall} = await this.checkAutInstallationState(
        // @ts-expect-error - do not assign arbitrary properties to `this.opts`
        {enforceAppInstall, fullReset: false, noReset: false, bundleId, device: this.opts.device, app: srcAppPath}
      );

      if (!install) {
        this.log.info(`Skipping the installation of '${srcAppPath}'`);
        return;
      }

      if (this.isRealDevice()) {
        // @ts-expect-error - do not assign arbitrary properties to `this.opts`
        await installToRealDevice(this.opts.device, srcAppPath, bundleId, {
          skipUninstall,
          timeout: timeoutMs ?? this.opts.appPushTimeout,
          strategy: strategy ?? this.opts.appInstallStrategy,
        });
      } else {
        // @ts-expect-error - do not assign arbitrary properties to `this.opts`
        await installToSimulator(this.opts.device, srcAppPath, bundleId, {
          skipUninstall
        });
      }
    }
