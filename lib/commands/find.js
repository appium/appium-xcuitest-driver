...
helpers.findNativeElementOrElements = async function (strategy, selector, mult, context) {
  ...
  if (strategy === 'xpath' && MAGIC_FIRST_VIS_CHILD_SEL.test(selector)) {
    [strategy, selector] = rewriteMagicFirstVisChild(mult);
...
function rewriteMagicFirstVisChild (mult) {
  if (mult) {
    throw new Error("Cannot get multiple first visible children!");
  }
  const strategy = 'class chain';
  const selector = '*[`visible == 1`][1]';
  log.info("Rewrote request for first visible child to class chain format " +
