...
export default {
   ...
   *
   * @since Xcode 15
   * @param {string} bundleId - The bundle identifier of target app. The app must be running
   * @this {XCUITestDriver}
   * @throws {Error} if app was not running
   */
  async mobileSendMemoryWarning(bundleId) {
    ...
    const pids = (await devicectl.listProcesses())
      ...
      .filter(({executable}) => new RegExp(`^${_.escapeRegExp(appInfo.url)}[^/]+$`).test(executable))
