...
async function closeRemoteXPC(self) {
  if (self._remoteXPCConditionInducerConnection) {
    await self._remoteXPCConditionInducerConnection.remoteXPC.close();
    self._remoteXPCConditionInducerConnection = null;
  }
}
...
export async function listConditionInducers() {
  ...
  if (isIos18OrNewer(this.opts)) {
    ...
    try {
      ...
      log.error(`Failed to list condition inducers via RemoteXPC: ${err.message}`);
      throw err;
    } finally {
...
export async function enableConditionInducer(conditionID, profileID) {
  ...
  if (isIos18OrNewer(this.opts)) {
    ...
    try {
      ...
      log.info(`Successfully enabled condition profile: ${profileID}`);
      return true;
    } catch (err) {
      await closeRemoteXPC(this);
      log.error(`Failed to enable condition inducer via RemoteXPC: ${err.message}`);
      throw this.log.errorWithException(`Enable condition inducer error: '${err.message}'`);
    }
  }
...
export async function disableConditionInducer() {
  if (isIos18OrNewer(this.opts)) {
    if (!this._remoteXPCConditionInducerConnection) {
      log.warn('Condition inducer connection is not active');
      return false;
    }

    try {
      await this._remoteXPCConditionInducerConnection.conditionInducer.disable();
      log.info('Successfully disabled condition inducer');
      return true;
    } catch (err) {
      log.warn(`Failed to disable condition inducer via RemoteXPC: ${err.message}`);
      return false;
    } finally {
      log.info(`Closing remoteXPC connection for device ${this.device.udid}`);
      await closeRemoteXPC(this);
  ...
  if (!this._conditionInducerService) {
    log.warn('Condition inducer server is not started');
    return false;
  }
  try {
    const ret = await this._conditionInducerService.callChannel(
    ...
    if (!_.isBoolean(ret.selector)) {
      log.warn(`Disable condition inducer error: '${JSON.stringify(ret.selector)}'`);
      return false;
    }
    return ret.selector;
  } finally {
