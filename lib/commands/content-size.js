...
export default {
  ...
  async mobileSetContentSize(size) {
    ...
    if (!CONTENT_SIZE.includes(size)) {
      throw new errors.InvalidArgumentError(`The 'size' value is expected to be one of ${CONTENT_SIZE.join(',')} (case-sensitive)`);
    }

    await /** @type {import('../driver').Simulator} */ (this.device).setContentSize(size);
  },
  ...
  async mobileGetContentSize() {
    assertSimulator(this);
    return await /** @type {import('../driver').Simulator} */ (this.device).getContentSize();
  },
};
 ...
 * @typedef {import('./types').ContentSize} ContentSize
 */
