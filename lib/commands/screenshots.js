...
commands.getViewportScreenshot = async function getViewportScreenshot () {
  ...
  log.info(`scale in  'getViewportScreenshot': ${scale}`);
  // status bar height comes in unscaled, so scale it
  statusBarHeight = Math.round(statusBarHeight * scale);
  log.info(`statusBarHeight in  'getViewportScreenshot': ${statusBarHeight}`);
  const windowSize = await this.getWindowSize();
  log.info(`windowSize in  'getViewportScreenshot': ${windowSize}`);
  let rect = {
    left: 0,
    top: statusBarHeight,
