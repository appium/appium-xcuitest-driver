...
commands.getContentSize = async function (el) {
  if (this.isWebContext()) {
    throw new errors.NotYetImplementedError('Support for getContentSize for webcontext is not yet implemented. Please contact an Appium dev');
  }
  ...
  if (children.length === 1) {
    ...
    // call to WDA
    const rect = await this.getRect(_.first(children));
    ...
    switch (type) {
      case "XCUIElementTypeTable": {
        const firstRect = await this.getRect(_.first(children));
      ...
      case "XCUIElementTypeCollectionView": {
        let elsInRow = 0;
        let firstRect = await this.getRect(_.first(children));
