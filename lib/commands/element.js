...
commands.getAttribute = async function (attribute, el) {
  ...
  if (!this.isWebContext()) {
    const value = await this.proxyCommand(`/element/${el}/attribute/${attribute}`, 'GET');
...
commands.setValue = async function (value, el) {
  ...
  if (this.isWebContext()) {
    ...
    //   ['s', 'o', 'm', 'e', ' ', 't', 'e', 'x', 't']
    //   'some text'
    if (_.isNull(value) || _.isUndefined(value) || (_.isObject(value) && !_.isArray(value))) {
      throw new Error(`Invalid argument to setValue: '${JSON.stringify(value)}'`);
    }
    if (_.isArray(value)) {
