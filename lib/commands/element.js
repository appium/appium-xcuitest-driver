...
commands.setValue = async function (value, el) {
  ...
  if (this.isWebContext()) {
    ...
    const setFormattedValue = async (input, isKeyboardPresenceCheckEnabled) => {
      ...
      try {
        await this.proxyCommand(`/element/${el}/value`, 'POST', {value: input, maximumFrequency: this.opts.maxTypingFrequency});
      } catch (err) {
...
commands.clear = async function (el) {
  ...
  let params = {};
  if (util.hasValue(this.opts.maxTypingFrequency)) {
    params.maximumFrequency = this.opts.maxTypingFrequency;
  }
  await this.proxyCommand(`/element/${el}/clear`, 'POST', params);
};
