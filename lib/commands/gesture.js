...
commands.performTouch = async function (gestures) {
  ...
  const supportedGesturesMapping = {
    ...
    longPress: {
      ...
      matches: [
        [{action: 'longpress'}],
        [{action: 'longpress'}, {action: 'release'}],
    ...
    drag: {
      ...
      matches: [
        ...
        [{action: 'longpress'}, {action: 'moveTo'}, {action: 'release'}]
      ]
    },
    scroll: {
...
function parseIntParameter (paramName, paramValue, methodName) {
  if (_.isUndefined(paramValue)) {
    log.errorAndThrow(`"${paramName}" parameter is mandatory for "${methodName}" call`);
  }
...
helpers.mobileTapWithNumberOfTaps = async function (opts={}) {
  if (!opts.element) {
    opts.element = await this.findNativeElementOrElements(`class name`, `XCUIElementTypeApplication`, false);
  }
  let params = {
    taps: parseIntParameter('taps', opts.taps, 'tapWithNumberOfTaps'),
    touches: parseIntParameter('touches', opts.touches, 'tapWithNumberOfTaps')
  };
  const el = opts.element.ELEMENT || opts.element;
