...
helpers.gesturesChainToString = function (gestures) {
  return gestures.map((item) => {
    const otherKeys = _.difference(_.keys(item), ['action']);
    if (otherKeys.length) {
      return `${item.action}` +
...
commands.performTouch = async function (gestures) {
  log.debug(`Received the following touch action: ${this.gesturesChainToString(gestures)}`);
  ...
  for (let [cmd, info] of _.toPairs(supportedGesturesMapping)) {
    for (let candidateMatch of info.matches) {
      if (this.isSameGestures(gestures, candidateMatch)) {
  ...
  for (let [cmd, info] of _.toPairs(supportedGesturesMapping)) {
    ...
    for (let candidateMatch of info.matches) {
      availableGestures += `\t\t${this.gesturesChainToString(candidateMatch)}\n`;
    }
  }
  throw new errors.NotYetImplementedError(`Support for ${this.gesturesChainToString(gestures)} gesture is not implemented. ` +
