...
helpers.handleDrag = async function (gestures) {
  ...
  let duration;
  let moveToCoordinates;
  if (gestures.length === 4) {
    let press = gestures[0];
    let wait = gestures[1];
    let moveTo = gestures[2];

    pressCoordinates = await this.getCoordinates(press);
    duration = (parseInt(wait.options.ms, 10) / 1000);
    moveToCoordinates = await this.getCoordinates(moveTo);
  } else {
    let longPress = gestures[0];
    let moveTo = gestures[1];

    pressCoordinates = await this.getCoordinates(longPress);
    duration = (parseInt(longPress.options.duration, 10) / 1000);
    moveToCoordinates = await this.getCoordinates(moveTo);
  }
