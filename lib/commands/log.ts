...
import {NATIVE_WIN, isIos18OrNewerOrUnknown} from '../utils';
  ...
  if (_.isUndefined(this.logs.syslog)) {
    [this.logs.crashlog] = assignBiDiLogListener.bind(this)(
      new IOSCrashLog({
        ...
        log: this.log,
        useRemoteXPC: this.isRealDevice() && isIos18OrNewerOrUnknown(this.opts),
      }),
      {
        type: 'crashlog',
      },
    ...
    [this.logs.syslog] = assignBiDiLogListener.bind(this)(
      this.isRealDevice()
        ? new IOSDeviceLog({
            ...
            log: this.log,
            useRemoteXPC: isIos18OrNewerOrUnknown(this.opts),
          })
        : new IOSSimulatorLog({
            sim: this.device as Simulator,
            showLogs: this.opts.showIOSLog,
