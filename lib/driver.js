...
import { detectUdid, getAndCheckXcodeVersion, getAndCheckIosSdkVersion,
         ...
         DEFAULT_TIMEOUT_KEY } from './utils';
...
class XCUITestDriver extends BaseDriver {
  ...
  /**
   * Check whether the current driver is the first driver in the pipeline.
   *
   * @return {boolean} True if there are no more sessions instantiated from the current driver and false otherwise.
   *                   True by default.
   */
  get isFirstSession () {
    return _.isNull(this._isFirstSession) || this._isFirstSession;
  }
  ...
  /**
   * Adjust driver capabilities according to isFirstSession property value.
   *
   * @param {object} caps - Capabilities mapping for the current driver instance.
  ...
  _adjustSessionCapabilities (caps) {
    ...
    if (!this.isFirstSession && caps.allowTouchIdEnroll) {
      log.info(`Resetting 'allowTouchIdEnroll' capability, because it only makes sence for the very first session`);
      caps.allowTouchIdEnroll = false;
    }
  }

  async createSession (caps, reqCaps = null, otherSessionsData = []) {
    if (_.isNull(this._isFirstSession)) {
      // Set the value once on the first session creation
      this._isFirstSession = !(_.isArray(otherSessionsData) && otherSessionsData.length);
      if (this.isFirstSession) {
        log.info('Detected the current XCUITestDriver session is the first one in the pipeline');
      } else {
        log.info(`Detected the current XCUITestDriver session is running in parallel ` +
                 `with other ${otherSessionsData.length} session${otherSessionsData.length > 1 ? 's' : ''}`);
      }
      this._adjustSessionCapabilities(caps);
    }
  ...
  async determineDevice () {
    ...
    this.opts.deviceName = await translateDeviceName(this.opts.platformVersion, this.opts.deviceName);
