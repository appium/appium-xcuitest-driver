...
class XCUITestDriver extends BaseDriver {
  ...
  resetIos () {
    ...
    this.xcodeVersion = {};
    this.opts.iosSdkVersion = null; // Pass to xcodebuild
    this.contexts = [];
  ...
  async start () {
    ...
    if (!this.opts.webDriverAgentUrl && this.opts.iosSdkVersion) {
      // make sure that the xcode we are using can handle the platform
      if (parseFloat(this.opts.platformVersion) > parseFloat(this.opts.iosSdkVersion)) {
        let msg = `Xcode ${this.xcodeVersion.versionString} has a maximum SDK version of ${this.opts.iosSdkVersion}. ` +
  ...
  async determineDevice () {
    ...
    if (!this.opts.platformVersion && this.opts.iosSdkVersion) {
      log.info(`No platformVersion specified. Using latest version Xcode supports: '${this.opts.iosSdkVersion}' ` +
               `This may cause problems if a simulator does not exist for this platform version.`);
      this.opts.platformVersion = this.opts.iosSdkVersion;
    }
