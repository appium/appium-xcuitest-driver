...
export class XCUITestDriver extends BaseDriver {
  ...
  async start() {
    ...
    this.wda = new WebDriverAgent(
      ...
      {
        ...this.opts,
        // here needs to set properly if the given appium:derivedDataPath already exists. Or perhaps try to get `this.wda.retrieveDerivedDataPath()` without that case as well. Then, maybe also the `this.wda.retrieveDerivedDataPath()` result can be `derivedDataPath` for the WDA.
  ...
  async startWda() {
    ...
    if (this.opts.useXctestrunFile || this.opts.usePrebuiltWDA || !(await this.wda.isSourceFresh())) {
      ...
      this.log.info(`[debug]: ${derivedDataPath}`);
      if (derivedDataPath) {
