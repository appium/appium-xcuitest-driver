...
class XCUITestDriver extends BaseDriver {
  ...
  async checkAutInstallationState() {
    ...
    if (wasAppInstalled) {
      ...
      this.log.info(`App '${bundleId}' is not installed yet or it has an offload, ` +
        'which might keep the local data, cannot be detected.');
    }
  ...
  async installAUT() {
    ...
    if (install) {
      if (this.isRealDevice()) {
        const shouldEnforceUninstall = this.opts.enforceAppInstall !== false || this.opts.fullReset;
