    ...
    return await SHARED_RESOURCES_GUARD.acquire(synchronizationKey, async () => {
      ...
      await retryInterval(startupRetries, startupRetryInterval, async () => {
        ...
        if (this.cachedWdaStatus?.build) {
          this.log.info(`Running WDA version is '${this.cachedWdaStatus?.build?.version}'.`);
        } else {
          this.log.info(`Running WDA does not have version info. Old or customized one is running on the device.`);
        }
