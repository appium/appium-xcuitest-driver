...
class XCUITestDriver extends BaseDriver {
  ...
  async shouldSkipAutInstall () {
    ...
    if (result) {
      this.log.info(
        `The candidate version of ${this.opts.bundleId} is lower that the installed one ` +
        `(${candidateBundleVersion} <= ${appBundleVersion}). The app won't be reinstalled.`
      );
    } else {
      this.log.info(
        `The installed version of ${this.opts.bundleId} is lower that the candidate one ` +
