...
const defaultWDARunnerBundleID = 'com.facebook.WebDriverAgentRunner.xctrunner';
...
class XCUITestDriver extends BaseDriver {
  ...
  constructor(opts = /** @type {XCUITestDriverOpts} */ ({}), shouldValidateCaps = true) {
    ...
    // Used to manage XCTest process directly to manage preinstalled WDA process
    // instead of over this.wda.
    // TODO: move as a class to handle xctestWDA stuff.
    this.xctestWDA = null;
  }
    ...
    return await SHARED_RESOURCES_GUARD.acquire(synchronizationKey, async () => {
      ...
      const quitAndUninstall = async (msg) => {
        ...
        if (this.opts.webDriverAgentUrl) {
          ...
          this.log.debug(
            'Not uninstalling WebDriverAgent since usePreinstalledWDA capability is provided'
          );
          throw new Error(msg);
        }
      ...
      await retryInterval(startupRetries, startupRetryInterval, async () => {
        ...
        try {
          if (this.isRealDevice() && this.opts.usePreinstalledWDA) {
            this.cachedWdaStatus = await this.startXctestWda(sessionId);
          } else {
  ...
  /**
   * Start preinstalled WDA process via XCTest.
   *
   * @param {string} sessionId A session id to proxy requests to.
   * @return {object} State Object
   */
  async startXctestWda(sessionId) {
    const xctestEnv = {};
    if (this.opts.wdaLocalPort) {
      xctestEnv.USE_PORT = this.opts.wdaLocalPort;
    }

    const launchWDABundleID = this.opts.updatedWDABundleId || defaultWDARunnerBundleID;
    this.mobileKillApp(launchWDABundleID);

    this.xctestWDA = new Xctest(this.opts.udid, launchWDABundleID, null, {env: xctestEnv});
    await this.xctestWDA.start();

    this.wda.setupProxies(sessionId);
    this.wda.fullyStarted = true;
    return await this.wda.getStatus();
  }

  /**
   * Stop the running WDA process by this.xctestWDA.
   */
  stopXctestWda() {
    this.xctestWDA.stop();
    this.xctestWDA = null;
    if (this.wda.jwproxy) {
      this.wda.jwproxy.sessionId = null;
    }
    this.wda.fullyStarted = false;
  }
  ...
  async stop() {
    ...
    if (this.wda && this.wda.fullyStarted) {
      ...
      if (this.isRealDevice() && this.xctestWDA) {
        this.stopXctestWda();
      } else if (!this.wda.webDriverAgentUrl && this.opts.useNewWDA) {
        await this.wda.quit();
      }
    }
  ...
  COMMAND_MAP = commands.executeExtensions.COMMAND_MAP;
  ...
  background = commands.generalExtensions.background;
  ...
  /*---------+
   | GESTURE |
   +---------+*/
  moveTo = commands.gestureExtensions.moveTo;
