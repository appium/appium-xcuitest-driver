...
export class XCUITestDriver extends BaseDriver {
  ...
  async handleMjpegOptions() {
    ...
    await this.allocateMjpegServerPort(this.opts.udid,
      mjpegServerPort, this.isRealDevice() ? true : false);
  ...
  async allocateMjpegServerPort(udid, mjpegServerPort, usePortForwarding) {
    this.log.debug(
      `MJPEG broadcasting requested, forwarding MJPEG server port ${mjpegServerPort} ` +
      `to local port ${mjpegServerPort}`
    );
    try {
      await DEVICE_CONNECTIONS_FACTORY.requestConnection(udid, mjpegServerPort, {
  ...
  async stop() {
    ...
    DEVICE_CONNECTIONS_FACTORY.releaseConnection(this.opts.udid, this.opts.mjpegServerPort);
  }
