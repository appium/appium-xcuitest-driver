import { errors, BaseDriver, isSessionCommand } from 'appium-base-driver';
...
class WebDriverAgentDriver extends BaseDriver {
  ...
  async determineDevice () {
    let realDevice, udid;
    let existingSim = await getExistingSim(this.opts.deviceName, this.opts.platformVersion);
    if (this.opts.udid) {
      ...
      if (this.opts.reset || this.opts.fullReset) {
        await existingSim.clean();
      }
      this.lifecycleData.createSim = false;
      udid = existingSim.udid;
      realDevice = false;
    } else {
