  ...
  async startWda(sessionId, realDevice) {
    ...
    if (this.opts.usePreinstalledWDA) {
      ...
      if (this.opts.prebuiltWDAPath && !(await fs.exists(this.opts.prebuiltWDAPath))) {
        throw new Error(`'prebuiltWDAPath' does not exist. Please make sure if the path exits.`);
      }
    }
        ...
        try {
          if (this.opts.usePreinstalledWDA) {
            ...
            if (this.opts.prebuiltWDAPath) {
              const candidateBundleVersion = await extractBundleVersion.bind(this)(
                this.opts.prebuiltWDAPath
              );
              this.log.info(`candidateBundleVersion -> ${candidateBundleVersion}`);
              this.log.info(`Installing prebuilt WDA ${this.opts.prebuiltWDAPath}`);
              // TODO: check if the test bundle does not have the version info. So maybe here should install every time.
              // TODO: get the bundle id from the 'this.opts.prebuiltWDAPath'
              const bundleId = this.opts.usePreinstalledWDA;
              await installToRealDevice(this.opts.device, this.opts.prebuiltWDAPath, bundleId, {
                skipUninstall: false,
