...
import { simBooted, createSim, getExistingSim } from './simulatorManagement';
...
import Fruitsrap from './fruitstrap';
...
class XCUITestDriver extends BaseDriver {
  ...
  async installToRealDevice () {
    ...
    if (this.opts.udid) {
      ...
      if (this.opts.ipa && this.opts.bundleId) {
        await this.opts.device.installAndWait(this.opts.ipa, this.opts.bundleId);
  ...
  async getIDeviceObj () {
    log.debug(`Creating Fruitsrap object with udid ${this.opts.udid}`);
    try {
      //keeping this iDevice for future once we have ideviceinstaller working for ios 10
      iDevice(this.opts.udid);
      let deviceAppInstaller = new Fruitsrap(this.opts.udid, this.opts.fruitstrapPath);
      if (!await fs.hasAccess(deviceAppInstaller.cmd)) {
        throw new Error(`fruitstrap is not located at ${deviceAppInstaller.cmd}. Make sure you pass right path with capability fruitstrapPath`);
      }
    } catch (e) {
      let msg = "Could not initialize Fruitsrap make sure it is " +
