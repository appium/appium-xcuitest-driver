...
export class XCUITestDriver extends BaseDriver {
  ...
  isWdaConnectionError(err) {
    ...
    return connectionErrors.some((errStr) => errorMessage.includes(errStr));
  }
  ...
  async reinitializeSession() {
    ...
    try {
      ...
      this.log.warn(`WDA process check failed: ${err.message}`);
      this.log.info('Restarting WDA process...');
    ...
    try {
      ...
      this.log.info(`WDA session reinitialized successfully, WDA session ID: ${this.sessionId}`);
      ...
      this.log.info(`WDA session reinitialized successfully, new WDA session ID: ${this.sessionId}`);
  ...
  async executeCommand(cmd, ...args) {
    this.log.debug(`Executing command_11111 '${cmd}'`);
    ...
    while (true) {
      try {
        ...
        const isWdaCrash = this.isWdaConnectionError(err) ||
          (err.message && (
            err.message.includes('invalid session id') ||
            err.message.includes('Cannot find any matching app') ||
        ...
        this.log.warn(`WDA appears to have crashed or become unresponsive. Attempt ${attempts}/${maxAttempts} to recover...`);
        ...
        try {
          ...
          try {
            await this.proxyCommand(`/session/${this.sessionId}/source`, 'GET');
            this.log.info('WDA connection restored, session still valid');
          } catch (sessionErr) {
            if (sessionErr.message &&
              (
                sessionErr.message.includes('invalid session id') ||
                sessionErr.message.includes('Cannot find any matching app') ||
                sessionErr.message.includes('Session does not exist')
              )) {
          ...
          // if (this.opts.bundleId) {
          //     await this.proxyCommand('/wda/apps/activate', 'POST', {
          //         bundleId: this.opts.bundleId,
          //         shouldTerminateApp: false
          //     });
          // }
        } catch (recoveryErr) {
          this.log.warn(`Reconnection failed: ${recoveryErr.message}`);
