...
class WebDriverAgentDriver extends BaseDriver {
  ...
  async createSession (caps) {
    ...
    try {
      ...
      if (this.isSafari() && this.isSimulator()) {
        ...
        this.safariDriver = await this.createSafariDriver();
      ...
      // create safari driver so enable querying webviews
      this.safariDriver = await this.createSafariDriver();
  ...
  shouldProxyToSafari (command) {
    return this.inWebview() &&
           ...
           !_.contains(this.getProxyAvoidList(), command);
  }

  async executeCommand (cmd) {
    if (this.shouldProxyToSafari(cmd)) {
      return await this.safariDriver.executeCommand(...arguments);
    }
