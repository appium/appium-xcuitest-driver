...
class XCUITestDriver extends BaseDriver {
  ...
  async start () {
    ...
    if (this.isSafari()) {
      ...
      if (!this.doesSafariSupportUrlArgument() && !this.isRealDevice()) {
        this.log.info(`About to set the initial URL to '${this._currentUrl}' on Simulator. ` +
          `Use 'safariInitialUrl' capability in order to customize it`);
        await this.opts.device.openUrl(this._currentUrl);
      }

      if (this.opts.autoWebview) {
        this.log.debug('Waiting for initial webview');
        await this.navToInitialWebview();
        this.logEvent('initialWebviewNavigated');
      }

      if (!this.doesSafariSupportUrlArgument() && this.isRealDevice()) {
        this.log.info(`About to set the initial URL to '${this._currentUrl}' on the device. ` +
          `Use 'safariInitialUrl' capability in order to customize it`);
