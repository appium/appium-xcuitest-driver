...
export class XCUITestDriver extends BaseDriver {
  ...
  async reconnectWda() {
    ...
    while (Date.now() - startTime < timeout) {
      try {
        ...
        if (isRunning) {
          this.log.info(`WDA is now reachable with status: ${isRunning}}`);
          return isRunning;
        } else {
          this.log.debug(`WDA still not reachable, status: ${isRunning}. Retrying in ${interval}ms...`);
    ...
    throw new Error(`Failed to reconnect to WDA within ${timeout}ms`);
  }
  ...
  async reinitializeSession() {
    ...
    try {
      ...
      this.log.error(`Failed to reinitialize WDA session: ${err.message}`);
      throw err;
    } finally {
      this._isRecovering = false;
    }
