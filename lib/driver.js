        ...
        try {
          ...
          if (this.isRealDevice() && this.opts.usePreinstalledWDA) {
            ...
            if (this.opts.wdaLocalPort) {
              env['USE_PORT'] = this.opts.wdaLocalPort

            }
            const launchWDABundleID = this.opts.updatedWDABundleId || 'com.facebook.WebDriverAgentRunner.xctrunner';
            // TODO: maybe here needs to quit already running WDA process like quitAndUninstall did?
            // TODO: should not uninstall the pre installed WDA when a wda process is running already. just kill the process.
            this.xcuitestWDA = new Xctest(this.opts.udid, launchWDABundleID, null, env);
            await this.xcuitestWDA.start();
            // should do something in this.wda like what this.webDriverAgentUrl does?
            this.wda.setupProxies(sessionId);
            this.wda.fullyStarted = true;
            // need something wait?
            this.cachedWdaStatus = await this.wda.getStatus();
          } else {
