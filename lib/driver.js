...
class WebDriverAgentDriver extends BaseDriver {
  ...
  shouldProxyToSafari (command) {
    ...
    return this.inWebview() &&
           ...
           !_.contains(commandsToNotProxyToSafari, command);
  }
  ...
  async executeCommand (cmd) {
    if (this.shouldProxyToSafari(cmd)) {
      ...
      //Don't proxy directly to wda, instead handle the command ourselves
      log.info(`${cmd} and ${arguments} DEBUG`);
