...
class XCUITestDriver extends BaseDriver {
  ...
  /**
   * @returns {boolean} if 'usePreinstalledWDA' capability is enabled.
   */
  usePreinstalledWDA() {
    return this.isRealDevice() && this.opts.usePreinstalledWDA;
  }
    ...
    return await SHARED_RESOURCES_GUARD.acquire(synchronizationKey, async () => {
      ...
      const quitAndUninstall = async (msg) => {
        ...
        if (this.opts.webDriverAgentUrl) {
          ...
          throw new Error(msg);
        } else if (this.usePreinstalledWDA()) {
          this.log.debug(
      ...
      await retryInterval(startupRetries, startupRetryInterval, async () => {
        ...
        try {
          ...
          this.cachedWdaStatus = await this.wda.launch.bind(this.wda)(sessionId, realDevice);
        } catch (err) {
          ...
          if (this.usePreinstalledWDA()) {
            // Kill the process if existed as teardown.
            await this.mobileKillApp(this.wda.bundleIdForXctest());
          } else {
  ...
  async stop() {
    ...
    if (this.wda && this.wda.fullyStarted) {
      ...
      if ((!this.wda.webDriverAgentUrl && this.opts.useNewWDA) || this.opts.usePreinstalledWDA) {
        await this.wda.quit();
      }
    }
