...
class XCUITestDriver extends BaseDriver {
  ...
  resetIos () {
    ...
    this.webElementsCache = new LRU({
      maxSize: WEB_ELEMENTS_CACHE_SIZE,
    });
  }
  ...
  async shouldSkipAutInstall () {
    if (this.opts.enforceAppInstall !== false || this.opts.fullReset) {
      return false;
    }
    ...
    const candidateBundleVersion = await extractBundleVersion.bind(this)(this.opts.app);
    ...
    const appBundleVersion = this.isRealDevice()
      ? (await this.opts.device.fetchAppInfo(this.opts.bundleId))?.CFBundleVersion
      : BUNDLE_VERSION_PATTERN.exec(await this.opts.device.simctl.appInfo(this.opts.bundleId))?.[1];
    ...
    let result;
    try {
      result = util.compareVersions(candidateBundleVersion, '<=', appBundleVersion);
    } catch (err) {
    ...
    if (result) {
      this.log.info(
        `The candidate version of ${this.opts.bundleId} is lower than the installed one ` +
        `(${candidateBundleVersion} <= ${appBundleVersion}). The app won't be reinstalled.`
      );
    } else {
      this.log.info(
        `The installed version of ${this.opts.bundleId} is lower than the candidate one ` +
  ...
  async installAUT () {
    ...
    if (!await this.shouldSkipAutInstall()) {
      if (this.isRealDevice()) {
