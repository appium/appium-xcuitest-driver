...
class WebDriverAgentDriver extends BaseDriver {
  ...
  async createSession (caps) {
    ...
    try {
      ...
      await this.startWdaSession(this.opts.app, this.opts.bundleId, caps);
  ...
  async startWdaSession(appPath, bundleId, caps) {
    let desired = {
      desiredCapabilities: {
        app: appPath,
        bundleId: bundleId
    ...
    if (caps.processArguments) {
      desired = this.prepareProcessArguments(desired, caps.processArguments);
    }
    await this.wda.jwproxy.command('/session', 'POST', desired);
  }

  prepareProcessArguments(desired, launchArguments) {
    var processArguments = {};
    var envRegExp = /(-e)\s(\w*)\s(\w*)/g;
    var argRegExp = /([\S]+)/g;

    // Environment variables
    if (launchArguments.match(envRegExp)) {
      processArguments['environment'] = {};
      launchArguments.match(envRegExp).forEach(function(element, index) {
        var env = element.split(/\s/);
        processArguments.environment[env[1]] = env[2];
        launchArguments = launchArguments.replace(element, "");
      });
    }

    // Launch arguments
    if(launchArguments.match(argRegExp)) {
      processArguments['arguments'] = [];
      launchArguments.match(argRegExp).forEach(function(element,index) {
        processArguments.arguments.push(element);
      });
    }

    desired.desiredCapabilities['environment'] = processArguments['environment'];
    desired.desiredCapabilities['arguments'] = processArguments['arguments'];
