  ...
  async startWda() {
    ...
    return await SHARED_RESOURCES_GUARD.acquire(synchronizationKey, async () => {
      ...
      await retryInterval(startupRetries, startupRetryInterval, async () => {
        ...
        // We don't restrict the version but to leave what version of WDA is running on the device for debugging purpose.
        if (this.cachedWdaStatus?.build) {
          this.log.info(`Running WebDriverAgent version is '${this.cachedWdaStatus.build.version}'.`);
        } else {
          this.log.info(`Running WebDriverAgent does not have version info. Old or customized WebDriverAgent is running on the device.`);
        }
