...
class XCUITestDriver extends BaseDriver {
  ...
  /**
   * @typedef {Object} CheckAutInstallationState
   * @property {boolean} install
   * @property {boolean} skipUninstall
   * /

  /**
   * @typedef {Object} CheckAutInstallationStateOption
   * @property {boolean} enforceAppInstall
   * @property {boolean} fullReset
   * @property {boolean} noReset
   * @property {string} bundleId
   * @property {IOSDeploy|object} device - real device object or simulator object
   * @property {string} app
   */
   ...
   *
   * @param {CheckAutInstallationStateOption} [opts]
   * @returns {Promise<CheckAutInstallationState>}
   */
  async checkAutInstallationState(opts) {
    const {enforceAppInstall, fullReset, noReset, bundleId, device, app} = opts ?? {};
  ...
  async installAUT() {
    ...
    const {install, skipUninstall} = await this.checkAutInstallationState(this.opts);
    if (install) {
      if (this.isRealDevice()) {
        // @ts-expect-error - do not assign arbitrary properties to `this.opts`
