...
class XCUITestDriver extends BaseDriver {
  ...
  isXcodebuildNeeded() {
    return !(CAP_NAMES_NO_XCODEBUILD_REQUIRED.find((x) => Boolean(this.opts[x])));
  }
    ...
    return await SHARED_RESOURCES_GUARD.acquire(synchronizationKey, async () => {
      ...
      const quitAndUninstall = async (msg) => {
        ...
        if (!this.isXcodebuildNeeded()) {
          this.log.debug(
            `Not quitting/uninstalling WebDriverAgent since at least one of ${CAP_NAMES_NO_XCODEBUILD_REQUIRED} capability is provided`,
          );
          throw new Error(msg);
        }
      ...
      if (this.isXcodebuildNeeded()) {
        ...
        this.log.debug(`Trying to start WebDriverAgent once since at least one of ${CAP_NAMES_NO_XCODEBUILD_REQUIRED} capability is provided`);
        startupRetries = 1;
      }
