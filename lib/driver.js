...
export class XCUITestDriver extends BaseDriver {
  ...
  constructor(opts = /** @type {XCUITestDriverOpts} */ ({}), shouldValidateCaps = true) {
    ...
    if (_.isNil(this.opts.enableWdaRestart)) {
      this.opts.enableWdaRestart = false;
    }
  }
  ...
  async start() {
    ...
    if (this.isSimulator()) {
      ...
      if (_.isBoolean(this.opts.calendarAccessAuthorized)) {
        ...
        const methodName = `${this.opts.calendarAccessAuthorized ? 'enable' : 'disable'
          }CalendarAccess`;
  ...
  isWdaConnectionError(err) {
    ...
    return CONNECTION_ERRORS.some((errStr) => errorMessage.includes(errStr));
  }

  /**
   * Reconnects to WDA within a specified timeout
   * @returns {Promise<object>} - The WDA status
   * @throws {Error} - If WDA does not become reachable within the timeout
   */
  async reconnectWda() {
    const timeout = this.opts.wdaConnectionTimeout || 15000;
    const interval = this.opts.wdaStartupRetryInterval || 1000;
    ...
    while (Date.now() - startTime < timeout) {
      try {
        const status = await this.proxyCommand('/status', 'GET');
        this.log.info(`WDA is now reachable with status: ${JSON.stringify(status)}`);
        return status;
      } catch (err) {
        this.log.debug(`WDA still not reachable: ${err.message}. Retrying in ${interval}ms...`);
        await this.delay(interval);
      }
    }

    throw new Error(`WDA did not become reachable within ${timeout}ms`);
  }
  ...
  /**
   * Reinitializes the WDA session
   * @returns {Promise<string>} - The session ID
   * @throws {Error} - If session reinitialization fails
   */
  async reinitializeSession() {
    this.log.info('Reinitializing WDA session mapping...');

    const originalSessionId = this.sessionId;
    const isRunning = await this.wda.isRunning();

    // Handle WDA process state
    if (isRunning) {
      this.log.info('WDA process is still running');
    } else {
      this.log.info('WDA process is not running. Restarting WDA process...');

      // Quit any stale WDA instance
      if (this.wda?.fullyStarted) {
        try {
          await this.wda.quit();
        } catch (quitErr) {
          this.log.warn(`Error quitting stale WDA: ${quitErr.message}`);
        }
      }

      // Start a new WDA instance
      try {
        await this.startWda();
      } catch (startErr) {
        this.log.error(`Failed to start WDA: ${startErr.message}`);
        throw startErr;
      }
    }
    ...
    try {
      ...
      // Restore proxy and session settings
      this.proxyReqRes = this.wda.proxyReqRes.bind(this.wda);
      this.jwpProxyActive = true;
      this.sessionId = originalSessionId;

      this.log.info(`WDA session reinitialized with ID: ${this.sessionId}`);

      // Attempt to restore previous context if not NATIVE_APP
      await this.restoreContext();

      if (!this.sessionId) {
        throw new Error('Session ID is unexpectedly null after reinitialization');
      }

      this.log.info(`WDA session reinitialization completed successfully`);
      return this.sessionId;
    } catch (err) {
      this.log.error(`Failed to initialize WDA session: ${err.message}`);
      throw err;
    } finally {
      this._isRecovering = false;
    }
  ...
  /**
   * Attempts to restore the previous webview context
   * @private
   * @returns {Promise<void>}
   */
  async restoreContext() {
    if (this._currentContext && this._currentContext !== 'NATIVE_APP') {
      try {
        this.log.info(`Attempting to restore webview context: ${this._currentContext}`);
        await this.activateRecentWebview();
      } catch (contextErr) {
        this.log.warn(`Failed to restore webview context: ${contextErr.message}`);
        this._currentContext = 'NATIVE_APP';
      }
    }
  }

  /**
   * Executes a command with automatic WDA recovery on failure
   * @param {string} cmd - Command to execute
   * @param {...any} args - Command arguments
   * @returns {Promise<any>} - Command result
   * @throws {Error} - If command execution fails after recovery attempts
   */
  async executeCommand(cmd, ...args) {
    ...
    const wdaRestartEnabled = Boolean(this.opts.enableWdaRestart) || true; // temp commit for testing

    if (!wdaRestartEnabled) {
      return await super.executeCommand(cmd, ...args);
    }

    const maxAttempts = this.opts.wdaStartupRetries || 5;
    let attempts = 0;
    ...
    while (true) {
      try {
        ...
        this.log.debug(`Error executing command '${cmd}': ${err.message}`);
        attempts++;

        const isWdaCrash = this.isWdaCrashOrSessionInvalid(err);

        if (!isWdaCrash || attempts > maxAttempts) {
          throw err;
        }

        this._isRecovering = true;
        this.log.warn(`WDA issue detected. Recovery attempt ${attempts}/${maxAttempts}...`);

        try {
          await this.handleWdaRecovery();
          this.log.info('Recovery successful, retrying original command');
        } catch (recoveryErr) {
          this.log.error(`Recovery attempt failed: ${recoveryErr.message}`);
          if (attempts >= maxAttempts) {
            throw new Error(
              `Failed to recover WDA after ${maxAttempts} attempts: ${recoveryErr.message}`,
            );
          }
        }
      }
  ...
  /**
   * Checks if an error indicates a WDA crash or invalid session
   * @private
   * @param {Error} err - The error to check
   * @returns {boolean} - True if the error indicates WDA crash or invalid session
   */
  isWdaCrashOrSessionInvalid(err) {
    if (this.isWdaConnectionError(err)) {
      return true;
    }
  ...
  /**
   * Handles WDA recovery by attempting reconnection or reinitialization
   * @private
   * @returns {Promise<void>}
   */
  async handleWdaRecovery() {
    try {
      this.log.info('Attempting to reconnect to WDA...');
      await this.reconnectWda();
      ...
      try {
        ...
        this.log.info('WDA connection restored, session still valid');
      } catch (sessionErr) {
        if (this.isWdaCrashOrSessionInvalid(sessionErr)) {
          this.log.info('WDA session is invalid, reinitializing session...');
      ...
      this.log.warn(`Reconnection failed: ${reconnectErr.message}`);
      this.log.info('Attempting full WDA reinitialization...');
      await this.reinitializeSession();
    } finally {
      this._isRecovering = false;
    }
  ...
  async startWdaSession(bundleId, processArguments) {
    ...
    const wdaCaps = {
      ...
      shouldTerminateApp: this._isRecovering ? false : (this.opts.shouldTerminateApp ?? true),
      forceAppLaunch: this._isRecovering ? false : (this.opts.forceAppLaunch ?? true),
