...
import { detectUdid, getAndCheckXcodeVersion, getAndCheckIosSdkVersion,
         adjustWDAAttachmentsPermissions, checkAppPresent, getDriverInfo } from './utils';
...
class XCUITestDriver extends BaseDriver {
  ...
  async start () {
    ...
    if (!this.xcodeVersion) {
      ...
      log.debug(`Xcode version set to '${this.xcodeVersion.versionString}' (tools v${this.xcodeVersion.toolsVersion})`);
    }
