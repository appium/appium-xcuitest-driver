...
class XCUITestDriver extends BaseDriver {
  ...
  async installAUT() {
    ...
    if (install) {
      if (this.isRealDevice()) {
        let shouldSkipUninstall = skipUninstall;
        // Real devices could return true for isAppInstalled function even the device has no the bundle id.
        // e.g. the app is offload app
        // Then, Appium wants to ensure the absent by calling remove method forcefully.
        if (this.opts.enforceAppInstall !== false || this.opts.fullReset) {
          this.log.debug(`Enforce to uninstall ${this.opts.bundleId} before the installation.`);
          shouldSkipUninstall = false;
        }

        // @ts-expect-error - do not assign arbitrary properties to `this.opts`
        await installToRealDevice(this.opts.device, this.opts.app, this.opts.bundleId, {
          skipUninstall: shouldSkipUninstall,
          timeout: this.opts.appPushTimeout,
          strategy: this.opts.appInstallStrategy,
        });
      } else {
