...
class XCUITestDriver extends BaseDriver {
  ...
  async start() {
    ...
    if (this.isSimulator()) {
      await this.startAndSetupSimulator();
  ...
  async startAndSetupSimulator() {
    ...
    if (_.isBoolean(this.opts.autoFillPasswords)) {
      this.log.info(`Setting autoFillPasswords to ${this.opts.autoFillPasswords}`);
      // @ts-expect-error - do not assign arbitrary properties to `this.opts`
      await this.opts.device.setAutoFillPasswords(this.opts.autoFillPasswords);
    }
