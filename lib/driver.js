...
import {WDA_RUNNER_BUNDLE_ID} from "./wda/utils";
...
class XCUITestDriver extends BaseDriver {
  ...
  async uninstallWDAIfRunningBundleIdIsDifferent () {
    ...
    if (util.hasValue(currentAppBundleID) && util.hasValue(this.opts.updatedWDABundleId) && this.opts.updatedWDABundleId !== currentAppBundleID) {
    ...
    } else if (util.hasValue(currentAppBundleID) && !util.hasValue(this.opts.updatedWDABundleId) && WDA_RUNNER_BUNDLE_ID !== currentAppBundleID) {
      ...
      const message = util.hasValue(currentAppBundleID)
          ? `Will reuse previously cached WDA instance at '${this.wda.url.href}' with '${currentAppBundleID}'`
          : `Will reuse previously cached WDA instance at '${this.wda.url.href}'`;
      log.info(`${message}. Set the wdaLocalPort capability to a value different from ${this.wda.url.port} if this is an undesired behavior.`);
  ...
  async startWda (sessionId, realDevice) {
    ...
    if (this.opts.useNewWDA) {
      ...
      await this.uninstallWDAIfRunningBundleIdIsDifferent();
    }
