...
class XCUITestDriver extends BaseDriver {
  ...
  async startWda (sessionId, realDevice) {
    ...
    if (this.opts.useNewWDA) {
      ...
      // Old WDA has no bundleId in the /status. In the case,we only can the WDA is running with `isRunning`.
      ...
      if (util.hasValue(currentAppBundleID) && (util.hasValue(this.opts.updatedWDABundleId)) && (this.opts.updatedWDABundleId !== currentAppBundleID)) {
        ...
        await this.wda.uninstall();
      } else if (util.hasValue(currentAppBundleID) && ('com.facebook.WebDriverAgentRunner' !== currentAppBundleID)) {
        //TODO: WDA_RUNNER_BUNDLE_ID 'com.facebook.WebDriverAgentRunner'
        log.info(`Will uninstall running WDA since it isn't default, '${'com.facebook.WebDriverAgentRunner'}', bundle id.`);
        await this.wda.uninstall();
      } else {
        let message;
        if (util.hasValue(currentAppBundleID)) {
          message = `Will reuse previously cached WDA instance at '${this.wda.url.href}' with '${currentAppBundleID}'`;
        } else {
          message = `Will reuse previously cached WDA instance at '${this.wda.url.href}'`;
        }
