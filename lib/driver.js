...
class WebDriverAgentDriver extends BaseDriver {
  ...
  async startWdaSession(appPath, bundleId, caps) {
    ...
    if (caps.processArguments) {
      desired = this.addLaunchArguments(desired, caps.processArguments);
    }
    await this.wda.jwproxy.command('/session', 'POST', desired);
  }

  addLaunchArguments(desired, launchArguments) {
    // TODO improve to handle environment variables etc..
    if(!~launchArguments.indexOf('-e') && ~launchArguments.indexOf('-')) {
      let argResult = [];
      for (let arg of launchArguments.split(' ')) {
        arg = arg.trim();
        if (arg.length) {
          argResult.push(arg);
        }
      }
      desired.desiredCapabilities['arguments'] = argResult;
    } else {
      log.error(`These processArguments are not implemented yet: '${launchArguments}'. Please use '-ARGUMENT' or '-ARGUMENT VALUE'.`);
    }
    return desired;
  }
