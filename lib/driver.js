...
class XCUITestDriver extends BaseDriver {
  ...
  async start () {
    ...
    if (_.isBoolean(this.opts.reduceTransparency)) {
      await this.setReduceTransparency(this.opts.reduceTransparency);
    }
  ...
  /**
   * Set reduceTransparency as 'isEnabled' only when the capabilities has 'reduceTransparency'
   * The call is ignored for real devices.
   * @param {?boolean} isEnabled Wether enable reduceTransparency
   */
  async setReduceTransparency (isEnabled) {
    if (this.isRealDevice() || !_.isBoolean(isEnabled)) {
      return;
    }

    this.log.info(`Setting reduceTransparency to ${isEnabled}`);
