...
class WebDriverAgentDriver extends BaseDriver {
  ...
  async executeCommand (cmd, ...args) {
    if (this.shouldProxyToSafari(cmd)) {
      ...
      return await safariDriver.executeCommand(...args);
    }
    //check to see if we are searching via predicate string
    var locatorStrategy = args[1].req.body.using;

    if ("-ios predicate string" === locatorStrategy) {
      //WebDriverAgent uses 'predicate string'
      args[1].req.body.using = "predicate string";
    }
