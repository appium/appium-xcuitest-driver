...
const { globalPerformanceCache, globalMemoryMonitor, globalPerformanceTimer } = require('./performance-optimizations');
...
const WEB_ELEMENTS_CACHE_SIZE = 1000; // Increased from 500 to accommodate more web elements
...
export class XCUITestDriver extends BaseDriver {
  ...
  constructor(opts = /** @type {XCUITestDriverOpts} */ ({}), shouldValidateCaps = true) {
    ...
    this.memoryMonitor = globalMemoryMonitor;
    this.performanceTimer = globalPerformanceTimer;
  }
  ...
  async createSession(w3cCaps1, w3cCaps2, w3cCaps3, driverData) {
    try {
      ...
      this.performanceTimer.start('sessionCreation');
      ...
      const sessionCreationTime = this.performanceTimer.end('sessionCreation');
      this.log.info(`Session creation completed in ${sessionCreationTime.toFixed(2)}ms`);

      // Log memory usage
      const memoryUsage = this.memoryMonitor.getMemoryUsage();
      this.log.debug(`Memory usage - Current: ${(memoryUsage.current.heapUsed / 1024 / 1024).toFixed(2)}MB, Peak: ${(memoryUsage.peak.heapUsed / 1024 / 1024).toFixed(2)}MB`);
  ...
  async startWda() {
    ...
    // Performance optimization: Start derived data path retrieval in background
    // This allows the expensive operation to run in parallel with other initialization
    const derivedDataPromise = this.wda.retrieveDerivedDataPath();
    ...
    if (this.opts.useXctestrunFile || !(await this.wda.isSourceFresh())) {
      ...
      const derivedDataPath = await derivedDataPromise;
      if (derivedDataPath) {
