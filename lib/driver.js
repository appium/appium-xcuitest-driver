...
export class XCUITestDriver extends BaseDriver {
  ...
  async start() {
    ...
    // Building the WDA project every time if 'appium:usePrebuiltWDA' was true.
    // The prebuildWDA could take full build time at the initial time, but it only could take
    // a few seconds if the project has been built properly. It won't require `appium:derivedDataPath`
    // has pre built WDA project by this flag. When `appium:derivedDataPath` is provided,
    // the build will occur with it. If no `appium:derivedDataPath` is provided,
    // the build will occur by xcodebuild's random path.
    let shouldPrebuildWDA = this.opts.usePrebuiltWDA;
    if (shouldPrebuildWDA) {
      // todo: check 'Build' directory existence with this.opts.derivedDataPath path
    }
