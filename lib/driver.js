...
class XCUITestDriver extends BaseDriver {
  ...
  /** @type {WDACapabilities|undefined} */
  wdaCaps;
  ...
  async installOtherApps(otherApps) {
    ...
    for (const otherApp of appPaths) {
      this.isRealDevice()
      // @ts-expect-error - do not assign arbitrary properties to `this.opts`
      ? await installToRealDevice(this.opts.device, otherApp, undefined, {
        skipUninstall: true, timeout: this.opts.appPushTimeout, strategy: this.opts.appInstallStrategy,
      })
      // @ts-expect-error - do not assign arbitrary properties to `this.opts`
  ...
  async getSession() {
    ...
    if (!this.wdaCaps) {
      this.wdaCaps = /** @type {WDACapabilities} */ (await this.proxyCommand('/', 'GET'));
    }
    ...
    /** @type {import('@appium/types').SingularSessionData<XCUITestDriverConstraints>} */
