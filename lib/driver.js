...
class XCUITestDriver extends BaseDriver {
  ...
  async startWda(sessionId, realDevice) {
    ...
    if (this.opts.usePreinstalledWDA && !this.isRealDevice()) {
      throw new Error(`'usePreinstalledWDA' capability must be against a real device`);
    }
    const usePreinstalledWDA = this.isRealDevice() && this.opts.usePreinstalledWDA;
    ...
    return await SHARED_RESOURCES_GUARD.acquire(synchronizationKey, async () => {
      ...
      } else if (!util.hasValue(this.wda.webDriverAgentUrl) || !usePreinstalledWDA) {
