...
class XCUITestDriver extends BaseDriver {
  ...
  async start() {
    ...
    if (this.isSafari()) {
      if (!isDeepLinkingSupported(this.opts.platformVersion) && shouldSetInitialUrl(this.opts)) {
        this.log.info(
  ...
  async startWdaSession(bundleId, processArguments) {
    ...
    if (this.isSafari() && isDeepLinkingSupported(this.opts.platformVersion) && shouldSetInitialUrl(this.opts)) {
      let logMsg = `The initial Safari URL will be set to ${this.getCurrentUrl()}`;
      if (!this.opts.safariInitialUrl) {
        logMsg += `. Use 'safariInitialUrl' capability in order to customize it.`;
      }
      this.log.info(logMsg);
      wdaCaps.initialUrl = this.getCurrentUrl();
    }
 ...
 * @returns {boolean}
 */
function isDeepLinkingSupported(platformVersion) {
  return Boolean(platformVersion && util.compareVersions(platformVersion, '>=', '16.4'));
}
 ...
 * @returns {boolean}
 */
function shouldSetInitialUrl(opts) {
