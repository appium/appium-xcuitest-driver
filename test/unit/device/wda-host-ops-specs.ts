...
import {assertWdaHostPlatformSupported, createWdaHostOps} from '../../../lib/device/wda-host-ops';
  ...
  describe('assertWdaHostPlatformSupported', function () {
    it('allows externally managed WDA on non-macOS hosts', function () {
      withPlatform('linux', () => {
        assertWdaHostPlatformSupported({
          opts: {webDriverAgentUrl: 'http://127.0.0.1:8100'},
        } as any);
      });
    });
    ...
    it('rejects default real-device WDA startup on non-macOS hosts', function () {
      withPlatform('win32', () => {
        ...
        ).to.throw(/default real-device WebDriverAgent startup strategy requires macOS/);
      });
    });
    ...
    it('rejects Xcode-only capabilities with non-macOS preinstalled WDA startup', function () {
      withPlatform('linux', () => {
        expect(() =>
          assertWdaHostPlatformSupported({
            opts: {platformVersion: '18.0', usePreinstalledWDA: true, usePrebuiltWDA: true},
            isRealDevice: () => true,
          } as any),
    ...
    it('allows RemoteXPC-eligible preinstalled WDA startup on non-macOS hosts', function () {
      withPlatform('linux', () => {
        assertWdaHostPlatformSupported({
          opts: {platformVersion: '18.0', usePreinstalledWDA: true},
          isRealDevice: () => true,
