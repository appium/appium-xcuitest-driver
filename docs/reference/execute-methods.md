...
This extension requires macOS developer tools (`xcrun xctrace` or `instruments`) and is not
available in non-macOS RemoteXPC/preinstalled-WDA execution modes.
...
Name | Type | Required | Description | Example
--- | --- | --- | --- | ---
format | string | no | The format specifier string. Read [DayJS documentation](https://day.js.org/docs/en/display/format) to get the full list of supported datetime format specifiers. The default format is `YYYY-MM-DDTHH:mm:ssZ`, which complies to ISO-8601 | `YYYY-MM-DD HH:mm:ss`
