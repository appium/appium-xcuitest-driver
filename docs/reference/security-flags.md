...
|`xctest_screen_record`|Allow [`mobile: startXCTestScreenRecording`](./execute-methods.md#mobile-startxctestscreenrecording) on a **real device** when **iOS 18+** **appium-ios-remotexpc** with **XCTestAttachment** is **not** available, so recordings cannot be deleted from the device after stop. Not required when deletion **is** available (same check the driver uses before stop).|
